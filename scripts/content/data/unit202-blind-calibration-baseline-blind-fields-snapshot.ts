/**
 * CC-17A: frozen snapshot of every CC-17 row's own blind-baseline-defining
 * fields (publicSpecificationAnchor/publicRangeAnchor/publicAssessmentAnchor/
 * transferablePrerequisiteJustification/blindBaselineRequirement/
 * blindBaselineDepth/blindBaselineRationale/blindConfidence), captured
 * immediately after CC-17A added existingPrivateCalibrationClaim data --
 * proving CC-17A's own populate-the-comparison-field work never altered any
 * blind-method conclusion (task section 3's strict separation rule).
 *
 * This snapshot is deliberately NOT auto-regenerated by the validator -- a
 * future package that DOES legitimately need to change a blind-baseline
 * conclusion (an explicitly escalated genuine defect, per CC-17A's own
 * governing instruction) must update this file by hand, in the same commit,
 * as a visible, reviewable diff -- never silently.
 */

export const unit202BlindCalibrationBaselinePreCc17aBlindFieldsSnapshot: Record<
  string,
  {
