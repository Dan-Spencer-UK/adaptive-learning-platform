/**
 * CC-17: Unit 202 Blind Calibration Baseline. A Project-Architect review
 * export -- see docs/architecture/evidence/CC-17-UNIT202-BLIND-
 * CALIBRATION-BASELINE.md for the full report, methodology, and
 * scalability analysis this ledger supports.
 *
 * BLINDNESS: every `blindBaselineRequirement`/`blindBaselineDepth`/
 * `blindBaselineRationale` field below was constructed using ONLY:
 * (1) the AC's own official title/verb wording; (2) official Range item
 * labels; (3) qualification level (Level 2) and command-verb/AC-
 * structure/prerequisite reasoning; (4) the approved technical-source
 * dossier (CC-15/CC-15A/CC-15B) for factual-truth ceilings. NONE of
 * these fields were constructed from, or checked against,
 * `cgTeachingWorksheetCalibration`, any handout/worksheet/tutor-answer
 * summary, any OFFICIAL_TEACHING_INTERPRETATION-basis knowledge
 * obligation, or any legacy lesson/assertion content -- those appear
 * ONLY in `existingPrivateCalibrationClaim` (tier 5, explicitly labelled
 * "UNVERIFIED CALIBRATION CLAIM:") and `legacyPropagationKeys`
 * (diagnostic tier 6), never cited by a blind* field. The validator
 * mechanically scans every blind* field for private-material vocabulary
 * as a backstop.
 *
 * `publicAssessmentAnchor` draws on the governed matrix's own
 * `publicSampleAssessmentCalibration` text, which is TIER 2 evidence
 * (public C&G sample-assessment material, e.g. "Sample A"/"Sample B") --
 * explicitly permitted, unlike `cgTeachingWorksheetCalibration` (tier 5,
 * proprietary), which this file never reads from.
 *
 * MATRIX RELATIONSHIP: this file was seeded from the candidate-
 * proposition list CC-16's audit ledger already identified (so nothing
 * is missed), but every blind* field was independently reconstructed
 * from tiers 1-4 only, per this package's own governing instruction
 * (task section 5) -- the matrix is compared against, never presumed
 * correct or copied from for the blind fields themselves.
 */

import type { BlindCalibrationBaseline } from "@alp/content-schema";

export const unit202BlindCalibrationBaseline: BlindCalibrationBaseline = {
  baselineTitle: "CC-17 Unit 202 Blind Calibration Baseline",
  derivedFromMatrix: "unit202DepthPerformanceMatrix (scripts/content/data/unit202-depth-performance-matrix.ts) -- for candidate-proposition identification and AC/Range structure only, never for blind-field content",
  methodologyNote:
    "Each row reconstructs, from official specification/AC wording, official Range item labels, public sample-" +
    "assessment evidence, Level-2/command-verb/prerequisite reasoning, and the approved technical-source dossier " +
    "only, what the transferable ALP course-construction methodology would conclude a learner needs -- built " +
    "blind to any proprietary C&G handout/worksheet/tutor-answer content. matrixComparison then records the " +
    "purely factual relationship between this blind reconstruction and the existing governed matrix (which had " +
    "access to Project-Architect-summarised proprietary material), never a verdict about which is correct.",
  rows: [
    // =====================================================================
    // AC1.1 -- Mathematical principles (LO1). Range: 6 items.
    // =====================================================================
    {
      calibrationKey: "ac1-1-fractions-percentages",
      acNumber: "1.1", loNumber: 1,
      ...
      learnerPerformanceType: "CALCULATE",
      ...
      matrixComparisonNotes: "Matrix's own requiredSupportingKnowledge names the same content ('Fractions/decimals/percentages and proportional reasoning'); CC-16's own trace found direct Range anchor and specific sample-assessment evidence.",
      projectArchitectCalibrationQuestions: [
        "Does the handout/worksheet require converting AWKWARD fractions (e.g. non-terminating decimals) or only simple ones?",
        "Is proportional reasoning (ratios, direct/inverse proportion) taught as a distinct sub-topic, or folded entirely into percentage work?",
        "Does the worksheet's percentage work go beyond percentage-of-a-quantity into percentage error/tolerance calculations?",
      ],
    },
    {
      calibrationKey: "ac1-1-algebra-transposition",
      acNumber: "1.1", loNumber: 1,
    ...
    {
      calibrationKey: "ac1-1-indices-and-notation",
      acNumber: "1.1", loNumber: 1,
      rangeItems: ["Indices"],
      publicSpecificationAnchor: "AC1.1: 'Identify and apply appropriate mathematical principles...'",
      ...
      blindUncertaintyReason: "The bare Range label 'Indices' does not itself specify whether fractional/negative-index (root) forms are in scope, or only whole-number multiplication/division-of-powers and scientific notation -- no public sample-assessment evidence for this item was located to resolve it.",
      matrixComparison: "MATRIX_BROADER",
      matrixComparisonNotes: "Matrix's requiredSupportingKnowledge includes 'powers, roots' explicitly (implying fractional indices are in scope) -- this specific resolution is not independently derivable from the bare Range label alone under the blind method.",
      projectArchitectCalibrationQuestions: [
        "Does the worksheet test fractional/negative indices (e.g. x^(1/2) as a root), or only whole-number powers and scientific notation?",
        "Is there a sample-assessment item testing indices specifically that would resolve this depth question?",
      ],
    },
    {
      calibrationKey: "ac1-1-pythagoras-trig",
      acNumber: "1.1", loNumber: 1,
      ...
      projectArchitectCalibrationQuestions: [
        "Does the worksheet test all three ratios (sin/cos/tan) and Pythagoras, or emphasise one over the others?",
        "Are the triangle problems embedded in an electrical-installation context (e.g. conduit bend geometry) or presented as pure maths?",
      ],
    },
    {
      calibrationKey: "ac1-1-statistics-mean-median-mode",
      acNumber: "1.1", loNumber: 1,
      ...
      blindConfidence: "MEDIUM",
      blindUncertaintyReason: "The official Range names only the single word 'Statistics' with no sub-item breakdown, and no public sample-assessment evidence for this item was located -- whether the specific four-measure set (adding 'range' to mean/median/mode) or a different/broader set (e.g. including quartiles) is intended cannot be resolved from tiers 1-4 alone.",
      matrixComparison: "MATRIX_BROADER",
      matrixComparisonNotes: "Matrix's requiredSupportingKnowledge explicitly includes 'range' alongside mean/median/mode -- CC-16's audit found this specific four-item resolution in the current matrix rests on an OFFICIAL_TEACHING_INTERPRETATION-basis knowledge obligation citing a proprietary handout passage, which the blind method cannot use.",
      projectArchitectCalibrationQuestions: [
        "Does the handout explicitly name 'range' as a required statistical measure alongside mean/median/mode?",
        "Is 'range' tested in a worksheet or sample assessment, or only mentioned as context?",
        "Are quartiles or any other descriptive-statistics measure mentioned anywhere in the material?",
      ],
    },

    // =====================================================================
    // AC2.1 -- SI units of measurement (LO2). Range: 8 items.
    // =====================================================================
    {
      calibrationKey: "ac2-1-base-quantity-units",
      acNumber: "2.1", loNumber: 2,
      ...
      learnerPerformanceType: "IDENTIFY",
      ...
      factualSourceRequirementKeys: ["si-units-and-physical-quantities cluster (CC-15/CC-15A technical-source-verification)"],
      blindConfidence: "HIGH",
      matrixComparison: "SAME",
      matrixComparisonNotes: "Matches matrix content directly; each item independently Range-anchored.",
      projectArchitectCalibrationQuestions: [
        "Is prefix conversion (mm->m, mm^2->m^2) tested as a standalone skill, or only embedded within later calculation ACs?",
        "Are litre/m^3 volume conversions required, or only pure m^3?",
      ],
    },
    {
      calibrationKey: "ac2-1-temperature-kelvin-celsius",
      acNumber: "2.1", loNumber: 2,
      ...
      factualSourceRequirementKeys: ["SRC-BIPM-SI-9E-V4.01 (approved dossier FACTUAL_AUTHORITY)"],
      blindConfidence: "HIGH",
      matrixComparison: "SAME",
    ...
    // Special required case (task section 11.F): calculation-vs-
    // recognition depth for impedance/reactance/power-factor made explicit.
    // =====================================================================
    {
      calibrationKey: "ac2-2-quantity-symbol-unit-recognition",
      acNumber: "2.2", loNumber: 2,
      rangeItems: [
    ...
    {
      calibrationKey: "ac2-2-ac-quantity-calculation-depth-ceiling",
      acNumber: "2.2", loNumber: 2,
      ...
      learnerPerformanceType: "DISTINGUISH",
      ...
      projectArchitectCalibrationQuestions: [
        ...
        "Does the tutor-answer material show a full impedance calculation as an expected response, or only a definitional answer?",
      ],
    },
    ...
    // =====================================================================
    {
      calibrationKey: "ac2-3-instrument-selection-connection",
      acNumber: "2.3", loNumber: 2,
      ...
      factualSourceRequirementKeys: ["electrical-measurement-instruments cluster (CC-15/CC-15A technical-source-verification)"],
      blindConfidence: "HIGH",
      matrixComparison: "SAME",
      matrixComparisonNotes: "Matches matrix content directly.",
    ...
    {
      calibrationKey: "ac2-3-wattmeter-energy-meter",
      acNumber: "2.3", loNumber: 2,
    ...
    // =====================================================================
    // AC3.1 -- Mass and weight (LO3). No official Range list.
    // =====================================================================
    {
      calibrationKey: "ac3-1-mass-weight-definitions-relationship",
      acNumber: "3.1", loNumber: 3,
      ...
      factualSourceRequirementKeys: ["mass-and-weight cluster (CC-15/CC-15A technical-source-verification)"],
      blindConfidence: "HIGH",
      matrixComparison: "SAME",
    ...
    // =====================================================================
    // AC3.2 -- Levers, gears and pulleys (LO3). Special required case
    // (task section 11.D): explicit AC-wording-vs-Range-structure gap.
    // =====================================================================
    {
      calibrationKey: "ac3-2-lever-classes-and-balance",
      acNumber: "3.2", loNumber: 3,
      ...
      factualSourceRequirementKeys: ["simple-machines-levers-gears-pulleys cluster (CC-15A technical-source-verification, OpenStax torque/lever chapter)"],
      blindConfidence: "MEDIUM",
    ...
    {
      calibrationKey: "ac3-2-structural-gap-gears-pulleys-no-range-item",
      acNumber: "3.2", loNumber: 3,
      publicSpecificationAnchor: "AC3.2: 'Explain the principles of basic mechanics as they apply to levers, gears AND PULLEYS.'",
      ...
      blindConfidence: "HIGH",
      matrixComparison: "SAME",
      matrixComparisonNotes: "Identical finding to CC-16's own ac3-2-gears-and-pulleys-no-range-item row -- reported here again because it materially affects how the next two rows' evidence should be read (AC-title-only, not Range-anchored).",
      projectArchitectCalibrationQuestions: [
        "Does the private course material itself carry any structured Range-like breakdown for gears/pulleys, or does it also treat 'levers, gears and pulleys' as one undifferentiated AC3.2 topic?",
      ],
    },
    {
      calibrationKey: "ac3-2-gears",
      acNumber: "3.2", loNumber: 3,
      publicSpecificationAnchor: "AC3.2: 'Explain the principles of basic mechanics as they apply to levers, gears and pulleys.'",
      ...
      factualSourceRequirementKeys: ["simple-machines-levers-gears-pulleys cluster (CC-15A technical-source-verification, LibreTexts gear-systems)"],
      blindConfidence: "MEDIUM",
    ...
    },
    {
      ...
      factualSourceRequirementKeys: ["simple-machines-levers-gears-pulleys cluster (CC-15A technical-source-verification, OpenStax Physics 9.3 pulley passage)"],
      blindConfidence: "MEDIUM",
    ...
    // =====================================================================
    // AC3.3 / AC3.4 -- Force, work, energy, power, efficiency (LO3).
    // =====================================================================
    {
      calibrationKey: "ac3-3-mechanics-concepts",
      acNumber: "3.3", loNumber: 3,
      ...
      learnerPerformanceType: "DESCRIBE",
      ...
      blindConfidence: "MEDIUM",
      blindUncertaintyReason: "Whether the quantitative KE/PE formulae belong under AC3.3 (conceptual) or AC3.4 (calculation) cannot be resolved purely from command-verb analysis, since AC3.4 is a separate AC that could plausibly absorb this calculation depth instead -- see the AC3.4 row.",
      matrixComparison: "SAME",
      matrixComparisonNotes: "Matrix reaches an equivalent conclusion, explicitly excluding the 1/2mv^2 formula 'unless later assessment/source evidence specifically requires it' -- the blind method reaches this same conditional exclusion independently from command-verb reasoning.",
      projectArchitectCalibrationQuestions: [
        "Does the handout teach KE=1/2mv^2 and GPE=mgh as named formulae under this AC specifically, or defer them to the calculation AC (3.4)?",
        "Is the force-as-push-or-pull definition taught with named example types (contact, gravitational, etc.), or generically?",
      ],
    },
    {
      calibrationKey: "ac3-4-mechanics-calculation",
      acNumber: "3.4", loNumber: 3,
    ...
    // =====================================================================
    {
      calibrationKey: "ac4-1-atomic-structure-and-current",
      acNumber: "4.1", loNumber: 4,
      ...
      factualSourceRequirementKeys: ["electron-theory-and-conduction cluster (CC-15/CC-15A technical-source-verification)"],
      blindConfidence: "HIGH",
      matrixComparison: "SAME",
      matrixComparisonNotes: "Matches matrix content directly.",
    ...
    // =====================================================================
    {
      calibrationKey: "ac4-2-conductor-insulator-distinction",
      acNumber: "4.2", loNumber: 4,
      ...
      blindConfidence: "MEDIUM",
      blindUncertaintyReason: "AC4.2 carries no official Range box at all, so the COMPLETE required material-example list cannot be derived from tier-1 evidence; only the two examples public sample-assessment evidence (tier 2) happens to test (tungsten, porcelain) can be confidently included.",
      matrixComparison: "DIFFERENT_EMPHASIS",
      matrixComparisonNotes: "CC-16's audit found the current matrix's requiredSupportingKnowledge names the same two sample-assessment-tested examples (copper/tungsten, porcelain/glass/plastics) but the corresponding governed ASSERTION in the corpus names a DIFFERENT material set entirely (copper/aluminium, PVC/rubber, chosen for installation-cable relevance) -- a genuine divergence CC-16 already flagged, restated here because it bears directly on this row's own material-example uncertainty.",
      projectArchitectCalibrationQuestions: [
        "Which specific conductor/insulator material examples does the handout use -- does it match the sample-assessment set (copper/tungsten, porcelain/glass/plastics), the governed-assertion set (copper/aluminium, PVC/rubber), or a third set entirely?",
        "Are the examples chosen for general-physics relevance or specifically for electrical-installation-work relevance (cable materials)?",
      ],
    },
    ...
    // =====================================================================
    {
      calibrationKey: "ac4-3-resistance-resistivity-relationship",
      acNumber: "4.3", loNumber: 4,
      ...
      factualSourceRequirementKeys: ["resistance-and-resistivity cluster (CC-15/CC-15A technical-source-verification)"],
      blindConfidence: "HIGH",
      matrixComparison: "SAME",
      matrixComparisonNotes: "Matches matrix content directly.",
    ...
    // =====================================================================
    // AC4.4 / AC4.5 -- DC circuit theory (series/parallel) (LO4).
    // =====================================================================
    {
      calibrationKey: "ac4-4-series-parallel-concepts",
      acNumber: "4.4", loNumber: 4,
      ...
      learnerPerformanceType: "EXPLAIN",
      ...
      blindConfidence: "HIGH",
      matrixComparison: "SAME",
      matrixComparisonNotes: "Matches matrix content directly; matrix's own reviewFlag about sample-question labels blurring 4.3/4.4/4.5 is a proprietary-material-adjacent observation the blind method does not need, since command-verb pairing alone resolves the AC4.4-vs-4.5 depth split.",
      projectArchitectCalibrationQuestions: [
        "Is qualitative reasoning ('if resistance increases, what happens to current') tested separately from numeric calculation, or always combined in one question?",
      ],
    },
    {
      calibrationKey: "ac4-5-series-parallel-calculation",
      acNumber: "4.5", loNumber: 4,
      ...
      factualSourceRequirementKeys: ["dc-circuit-theory-series-parallel cluster (CC-15/CC-15A technical-source-verification)"],
      blindConfidence: "HIGH",
      matrixComparison: "SAME",
      matrixComparisonNotes: "Matches matrix content directly.",
    ...
    // =====================================================================
    {
      calibrationKey: "ac4-6-power-calculation",
      acNumber: "4.6", loNumber: 4,
      ...
      factualSourceRequirementKeys: ["dc-circuit-power cluster (CC-15/CC-15A technical-source-verification)"],
      blindConfidence: "HIGH",
      matrixComparison: "SAME",
      matrixComparisonNotes: "Matches matrix content directly.",
    ...
    // =====================================================================
    {
      calibrationKey: "ac4-7-voltage-drop",
      acNumber: "4.7", loNumber: 4,
    ...
    // =====================================================================
    {
      calibrationKey: "ac4-8-thermal-chemical-effects",
      acNumber: "4.8", loNumber: 4,
      ...
      factualSourceRequirementKeys: ["thermal-and-chemical-effects-of-current cluster (CC-15A technical-source-verification, includes the dedicated fuse-operation passage)"],
      blindConfidence: "HIGH",
      matrixComparison: "SAME",
    ...
    // =====================================================================
    {
      calibrationKey: "ac5-1-attraction-repulsion-field-lines",
      acNumber: "5.1", loNumber: 5,
      ...
      factualSourceRequirementKeys: ["magnetism-flux-and-flux-density cluster (CC-15/CC-15A technical-source-verification)"],
      blindConfidence: "MEDIUM",
    ...
    // =====================================================================
    {
      calibrationKey: "ac5-2-flux-flux-density-relationship",
      acNumber: "5.2", loNumber: 5,
      ...
      factualSourceRequirementKeys: ["magnetism-flux-and-flux-density cluster (CC-15/CC-15A/CC-15B technical-source-verification)"],
      blindConfidence: "HIGH",
      matrixComparison: "SAME",
      matrixComparisonNotes: "Matches matrix content directly.",
    ...
    // =====================================================================
    // AC5.3 -- Magnetic effects of current (LO5). Special required case
    // (task section 11.C): 10 distinct items separated explicitly.
    // =====================================================================
    {
      calibrationKey: "ac5-3-field-around-conductor",
      acNumber: "5.3", loNumber: 5,
      ...
      factualSourceRequirementKeys: ["electromagnetism-motor-effect-and-induced-emf cluster (CC-15/CC-15A technical-source-verification)"],
      blindConfidence: "HIGH",
      matrixComparison: "SAME",
      matrixComparisonNotes: "Matches matrix content directly.",
      projectArchitectCalibrationQuestions: ["Is this fact taught as a standalone statement, or always immediately paired with the directional rule (next row)?"],
    },
    {
      calibrationKey: "ac5-3-field-direction-rule",
      acNumber: "5.3", loNumber: 5,
      publicSpecificationAnchor: "AC5.3: '...production of a magnetic field...' (direction is not itself named by the AC title).",
      learnerPerformanceType: "DIRECTION_RULE",
      ...
      factualSourceRequirementKeys: ["electromagnetism-motor-effect-and-induced-emf cluster (CC-15/CC-15A technical-source-verification, straight-wire section)"],
      blindConfidence: "MEDIUM",
    ...
    {
      calibrationKey: "ac5-3-force-on-conductor",
      acNumber: "5.3", loNumber: 5,
      ...
      factualSourceRequirementKeys: ["electromagnetism-motor-effect-and-induced-emf cluster (CC-15/CC-15A technical-source-verification, current-conductor-force section)"],
      blindConfidence: "MEDIUM",
    ...
    {
      calibrationKey: "ac5-3-flemings-left-hand-rule",
      acNumber: "5.3", loNumber: 5,
    ...
    {
      calibrationKey: "ac5-3-induced-emf",
      acNumber: "5.3", loNumber: 5,
      ...
      factualSourceRequirementKeys: ["electromagnetism-motor-effect-and-induced-emf cluster (CC-15/CC-15A technical-source-verification, motional-EMF section)"],
      blindConfidence: "MEDIUM",
    ...
    {
      calibrationKey: "ac5-3-flemings-right-hand-rule",
      acNumber: "5.3", loNumber: 5,
    ...
    {
      calibrationKey: "ac5-3-coil-solenoid-field",
      acNumber: "5.3", loNumber: 5,
      ...
      factualSourceRequirementKeys: ["electromagnetism-motor-effect-and-induced-emf cluster (CC-15/CC-15A technical-source-verification, solenoid section)"],
      blindConfidence: "LOW",
      blindUncertaintyReason: "No tier-1 (AC/Range) anchor at all -- 'coil' and 'solenoid' are absent from AC5.3's own title, and this AC carries no Range box. This row exists only because it is a plausible, generic-physics elaboration of the AC's named 'production of a magnetic field' topic, not because tiers 1-4 positively require it.",
      matrixComparison: "SAME",
      matrixComparisonNotes: "Matrix includes solenoid field/polarity content; CC-16's audit found this specific matrix record itself conditionally gapped (the word 'electromagnet' not confirmed in the approved technical source), broadly consistent with this row's own LOW-confidence, weak-anchor finding.",
      projectArchitectCalibrationQuestions: [
        "Is coil/solenoid field content taught as a distinct lesson topic, or only briefly as a lead-in to the electromagnet/relay/contactor application (next rows)?",
      ],
    },
    {
      calibrationKey: "ac5-3-electromagnet",
      acNumber: "5.3", loNumber: 5,
      publicSpecificationAnchor: "AC5.3 title does not itself name 'electromagnet'.",
      learnerPerformanceType: "COMPONENT_ROLE",
      ...
      blindConfidence: "LOW",
      blindUncertaintyReason: "No tier-1 anchor of any kind (not in the AC title, no Range box for this AC). This row is included only because it is a plausible, common application of the coil/solenoid content above, explicitly named as a candidate in the task's own required-case list -- not because the transferable methodology positively derives it.",
      matrixComparison: "MATRIX_ONLY_PROPOSITION",
      matrixComparisonNotes: "The current governed matrix includes 'electromagnet/relay/contactor basic principle' as required content under AC5.3; CC-16's audit already found NO anchor at all for this content in the matrix's own text (not the AC title, no Range box, no worksheet/sample-assessment claim, and no corresponding entry in the governed knowledge-obligations file) -- the blind method independently reaches the identical no-anchor finding.",
      projectArchitectCalibrationQuestions: [
        "Is 'electromagnet' taught as its own named topic/example under this AC, and if so with what level of technical detail (e.g. core material, turns-count effect)?",
      ],
    },
    {
      calibrationKey: "ac5-3-relay",
      acNumber: "5.3", loNumber: 5,
      ...
      blindConfidence: "LOW",
      blindUncertaintyReason: "No tier-1 anchor anywhere in the matrix for this specific named component under AC5.3.",
      matrixComparison: "MATRIX_ONLY_PROPOSITION",
      matrixComparisonNotes: "Same as 'electromagnet' above -- the current matrix bundles 'relay' into the same unanchored requiredSupportingKnowledge phrase, which CC-16's audit already found has no located evidence of any kind (not even a corresponding knowledge-obligations entry).",
      projectArchitectCalibrationQuestions: [
        "Is a relay specifically named and diagrammed under this AC, or only mentioned as one example among several electromagnet applications?",
        "Is relay content perhaps taught under AC6.1 instead (heating/boiler control switching/relay chain) rather than AC5.3?",
      ],
    },
    {
      calibrationKey: "ac5-3-contactor",
      acNumber: "5.3", loNumber: 5,
      ...
      blindConfidence: "LOW",
      blindUncertaintyReason: "No tier-1 anchor anywhere for this specific named component.",
      matrixComparison: "MATRIX_ONLY_PROPOSITION",
      matrixComparisonNotes: "Weakest-anchored of the four coil/solenoid/electromagnet/relay/contactor cluster -- 'contactor' is the most specific, most industrial-vocational-trade-specific term of the four, and the transferable methodology has the least confidence any of these four beyond basic coil/solenoid field content is genuinely required by C&G's own specification wording.",
      projectArchitectCalibrationQuestions: [
        "Is 'contactor' used anywhere in the teaching material at all, or is 'relay' the only such component named?",
        "If both relay and contactor appear, is the distinction between them (switching capacity/duty) explicitly taught, or are the terms used interchangeably?",
      ],
    },
    ...
    // =====================================================================
    {
      calibrationKey: "ac5-4-alternator-principle-and-parts",
      acNumber: "5.4", loNumber: 5,
      ...
      factualSourceRequirementKeys: ["ac-generation-single-loop-alternator cluster (CC-15/CC-15A technical-source-verification)"],
      blindConfidence: "HIGH",
      matrixComparison: "SAME",
      matrixComparisonNotes: "Matches matrix content directly.",
    ...
    {
      calibrationKey: "ac5-4-frequency-pole-pair-relationship",
      acNumber: "5.4", loNumber: 5,
      ...
      factualSourceRequirementKeys: ["ac-generation-single-loop-alternator cluster (CC-15A technical-source-verification, pole-pair convention cross-check)"],
      blindConfidence: "HIGH",
      matrixComparison: "SAME",
    ...
    // =====================================================================
    // AC5.5 -- Sine-wave characteristics (LO5). Range: 6 items.
    // =====================================================================
    {
      calibrationKey: "ac5-5-waveform-characteristics-and-relationships",
      acNumber: "5.5", loNumber: 5,
      ...
      factualSourceRequirementKeys: ["sine-wave-characteristics cluster (CC-15/CC-15A/CC-15B technical-source-verification)"],
      blindConfidence: "HIGH",
      matrixComparison: "SAME",
    ...
    // =====================================================================
    // AC6.1 -- Electronic systems and applications (LO6). Range: 6 items.
    // Special required cases A (telephones) and B (security alarms).
    // =====================================================================
    {
      calibrationKey: "ac6-1-security-alarms-category",
      acNumber: "6.1", loNumber: 6,
      ...
      blindConfidence: "HIGH",
      matrixComparison: "SAME",
      matrixComparisonNotes: "Matrix likewise treats 'Security alarms' as a directly Range-anchored category.",
      projectArchitectCalibrationQuestions: [
        "Is 'security alarm' taught purely as a category label with one worked example, or are multiple alarm-sensing technologies compared (beam-break, loop-break, motion)?",
      ],
    },
    {
      calibrationKey: "ac6-1-security-alarm-transistor-switching-generic",
      acNumber: "6.1", loNumber: 6,
      rangeItems: ["Security alarms"],
      ...
      blindConfidence: "MEDIUM",
      blindUncertaintyReason: "While transistor-switching-in-general is HIGH confidence (AC6.2), its specific APPLICATION to security alarms specifically (rather than, say, dimmer switches or motor control, which also involve switching) is an inference from category-combination reasoning, not a direct citation naming 'transistor' under the 'Security alarms' Range item.",
      matrixComparison: "SAME",
      matrixComparisonNotes: "Matrix's own requiredSupportingKnowledge does name 'transistor switching' under security alarms specifically -- the blind method reaches a compatible, if less specific, conclusion via cross-AC component-capability reasoning.",
      projectArchitectCalibrationQuestions: [
        "Does the handout use a transistor specifically for the sensing/switching role, or a different component (e.g. a comparator IC, a relay directly)?",
      ],
    },
    {
      calibrationKey: "ac6-1-security-alarm-thyristor-latching-generic",
      acNumber: "6.1", loNumber: 6,
      rangeItems: ["Security alarms"],
    ...
    {
      calibrationKey: "ac6-1-security-alarm-exact-topology",
      acNumber: "6.1", loNumber: 6,
      rangeItems: ["Security alarms"],
      ...
      blindConfidence: "LOW",
      blindUncertaintyReason: "No tier-1/tier-2 evidence can distinguish one specific representative topology from another electrically-equivalent one; this is inherently a course-design/exemplar CHOICE, not a fact derivable from the specification.",
      matrixComparison: "MATRIX_ONLY_PROPOSITION",
      matrixComparisonNotes: "The current governed matrix DOES specify one exact topology (transistor senses NC-loop break, triggers thyristor gate, latches sounder) -- CC-16's audit already found this specific choice traces to a proprietary-handout-interpretation basis in the governed knowledge obligations, with its own factual circuit-topology sourcing coming from a general electronics tutorial site, not a C&G artefact or first-party manufacturer document. The blind method independently confirms it cannot derive this SPECIFIC choice from transferable evidence alone, corroborating CC-16's finding from the opposite direction.",
      projectArchitectCalibrationQuestions: [
        "What EXACT topology does the private handout show for the security-alarm example -- does it match 'transistor senses NC-loop break -> triggers thyristor gate -> latches sounder' precisely, or a different arrangement?",
        "Is a circuit diagram shown at all, or only a functional block description?",
        "Is the loop normally-closed (broken = alarm) or normally-open (made = alarm)?",
      ],
    },
    {
      calibrationKey: "ac6-1-telephone-category",
      acNumber: "6.1", loNumber: 6,
      ...
      blindConfidence: "HIGH",
      matrixComparison: "SAME",
      matrixComparisonNotes: "Matches matrix's own direct Range anchor for the category.",
      projectArchitectCalibrationQuestions: [
        "Is 'telephone' still a live example in current teaching material, or flagged/being phased out given the UK PSTN-to-digital transition?",
      ],
    },
    {
      calibrationKey: "ac6-1-telephone-capacitor-role",
      acNumber: "6.1", loNumber: 6,
      rangeItems: ["Telephones"],
      publicSpecificationAnchor: "AC6.1: 'Describe the function and application of electronic components...'",
      learnerPerformanceType: "COMPONENT_ROLE",
    ...
    {
      calibrationKey: "ac6-1-telephone-resistor-role",
      acNumber: "6.1", loNumber: 6,
      rangeItems: ["Telephones"],
      publicSpecificationAnchor: "AC6.1: 'Describe the function and application of electronic components...'",
      learnerPerformanceType: "COMPONENT_ROLE",
      ...
      blindConfidence: "LOW",
      blindUncertaintyReason: "No tier-1/tier-2 evidence identifies this specific component role as required.",
      matrixComparison: "MATRIX_ONLY_PROPOSITION",
      matrixComparisonNotes: "Same finding as the capacitor row -- current matrix specifies this role, tracing to the same proprietary-interpretation-basis obligation and encyclopedia-sourced assertion CC-16's audit already found.",
      projectArchitectCalibrationQuestions: [
        "Does the handout name a resistor's specific role (e.g. remote line-testing / 'out-of-service' resistor) at all?",
      ],
    },
    {
      calibrationKey: "ac6-1-telephone-surge-protector-role",
      acNumber: "6.1", loNumber: 6,
      rangeItems: ["Telephones"],
      publicSpecificationAnchor: "AC6.1: 'Describe the function and application of electronic components...'",
      learnerPerformanceType: "COMPONENT_ROLE",
      ...
      blindConfidence: "LOW",
      blindUncertaintyReason: "No tier-1/tier-2 evidence identifies this component role as required, and it may additionally be a legacy/superseded design detail even if it once was.",
      matrixComparison: "MATRIX_ONLY_PROPOSITION",
      matrixComparisonNotes: "Weakest-evidenced of the three telephone component-role clauses per CC-16's own prior audit (no worksheet/sample-assessment claim of any kind located, not even a generic handout reference) -- the blind method's independent LOW-confidence finding is consistent with, and reinforces, that prior finding from a different evidentiary direction.",
      projectArchitectCalibrationQuestions: [
        "Does the handout mention a surge protector at all, and if so does it note this is specific to OLDER master sockets rather than universal?",
      ],
    },
    {
      calibrationKey: "ac6-1-telephone-master-vs-extension-distinction",
      acNumber: "6.1", loNumber: 6,
      rangeItems: ["Telephones"],
      publicSpecificationAnchor: "AC6.1: 'Describe the function and application of electronic components...'",
      learnerPerformanceType: "DISTINGUISH",
      ...
      blindConfidence: "LOW",
      blindUncertaintyReason: "No tier-1/tier-2 evidence of any kind, and arguably not even the right KIND of content for this AC's own 'electronic components' framing.",
      matrixComparison: "MATRIX_ONLY_PROPOSITION",
      matrixComparisonNotes: "CC-16's audit already found this to be the single weakest-evidenced telephone clause of all -- not even present in the governed knowledge-obligation's own description text, only one level further removed in the underlying assertion's own statement. The blind method's independent finding (weakest of five telephone rows, and arguably off-topic for the AC's own framing) corroborates this from a different angle.",
      projectArchitectCalibrationQuestions: [
        "Is the master-vs-extension distinction taught at all, and if so, is it framed as electronic-component content or as wiring/installation-practice content (which might belong to a different unit entirely)?",
      ],
    },

    {
      calibrationKey: "ac6-1-telephone-other-component-detail-check",
      acNumber: "6.1", loNumber: 6,
      rangeItems: ["Telephones"],
      publicSpecificationAnchor: "AC6.1: 'Describe the function and application of electronic components...'",
      learnerPerformanceType: "COMPONENT_ROLE",
      ...
      blindConfidence: "LOW",
      blindUncertaintyReason: "By construction this is a check for the ABSENCE of further tier-1/tier-2 evidence, which cannot itself be HIGH confidence -- private material could legitimately cover additional telephone components this audit has no way to anticipate.",
      matrixComparison: "MATRIX_ONLY_PROPOSITION",
      matrixComparisonNotes: "CC-16's own audit carried a matching catch-all row (ac6-1-telephone-other-detail-check) recording that it found no further matrix-claimed telephone component detail beyond the same four clauses -- this row's independent, tiers-1-4-only check reaches the same 'nothing further located' conclusion.",
      projectArchitectCalibrationQuestions: [
        "Does the private material cover any telephone-internal component beyond capacitor/resistor/surge-protector/master-vs-extension wiring (e.g. hook-switch, ringer transducer, hybrid transformer, dialling circuit)?",
      ],
    },

    // =====================================================================
    // AC6.1 -- remaining Range items (dimmer, heating/boiler, motor,
    // wireless). Each: category (Range-anchored, HIGH) is separated from
    // the specific representative topology (not independently derivable,
    // LOW), matching the security-alarm/telephone treatment above.
    // =====================================================================
    {
      calibrationKey: "ac6-1-dimmer-switch-category",
      acNumber: "6.1", loNumber: 6,
    ...
    {
      calibrationKey: "ac6-1-heating-boiler-control-category",
      acNumber: "6.1", loNumber: 6,
    ...
    {
      calibrationKey: "ac6-1-motor-control-category",
      acNumber: "6.1", loNumber: 6,
    ...
    {
      calibrationKey: "ac6-1-wireless-control-category",
      acNumber: "6.1", loNumber: 6,
      ...
      blindConfidence: "MEDIUM",
      blindUncertaintyReason: "The transmitter/receiver arrangement itself is near-definitional (HIGH), but WHICH 'practical advantages' are the expected required content (no wiring cost, retrofit ease, flexible placement, etc.) is not independently derivable from tiers 1-4 -- no sample-assessment evidence for this item was located either.",
      matrixComparison: "SAME",
      matrixComparisonNotes: "Matches matrix's transmitter/receiver-arrangement conclusion; the 'practical advantages' half is present in both, though CC-16's audit found the current matrix's own supporting evidence for that specific half is a single, fairly generic maintenance-cost claim from one manufacturer source, not dissimilar in strength to this row's own MEDIUM confidence.",
      projectArchitectCalibrationQuestions: [
        "What specific practical advantage(s) of wireless control does the handout name (installation cost, retrofit ease, flexibility, reliability)?",
        "Is a specific real product/system used as the illustrative example?",
      ],
    },

    // =====================================================================
    // AC6.2 -- Electronic component operating principles (LO6). Range: 13
    // items. Special required case (task section 11.E): generic principle
    // separated from physical appearance, symbol recognition, and
    // application-specific usage.
    // =====================================================================
    {
      calibrationKey: "ac6-2-generic-operating-principles",
      acNumber: "6.2", loNumber: 6,
      rangeItems: ["Capacitors", "Resistors", "Rectifiers", "Diodes", "Zener", "LED", "Photo", "Thermistors", "DIACs", "TRIACs", "Transistors", "Thyristors", "Inverters"],
      ...
      factualSourceRequirementKeys: ["electronic-components-operating-principles cluster (CC-15/CC-15A/CC-15B technical-source-verification)"],
      blindConfidence: "HIGH",
      matrixComparison: "SAME",
    ...
    {
      calibrationKey: "ac6-2-schematic-symbol-recognition",
      acNumber: "6.2", loNumber: 6,
      rangeItems: ["Capacitors", "Resistors", "Rectifiers", "Diodes", "Zener", "LED", "Photo", "Thermistors", "DIACs", "TRIACs", "Transistors", "Thyristors", "Inverters"],
      ...
      learnerPerformanceType: "PHYSICAL_RECOGNITION",
    ...
    {
      calibrationKey: "ac6-2-physical-appearance-recognition",
      acNumber: "6.2", loNumber: 6,
      rangeItems: ["Capacitors", "Resistors", "Rectifiers", "Diodes", "Zener", "LED", "Photo", "Thermistors", "DIACs", "TRIACs", "Transistors", "Thyristors", "Inverters"],
      ...
      blindConfidence: "LOW",
      blindUncertaintyReason: "No tier-1 or tier-2 evidence distinguishes 'physical appearance is required assessed knowledge' from 'physical appearance images are merely helpful teaching illustration, not itself assessed'.",
      matrixComparison: "MATRIX_BROADER",
      matrixComparisonNotes: "The current matrix's own visualRepresentationRequirement field does list 'physical appearance as supporting recognition where useful' -- notably hedged with 'where useful', suggesting even the matrix itself treats this as supportive/pedagogical rather than a hard requirement, broadly consistent with this row's own LOW-confidence, non-required finding. CC-16's audit independently reached the same conclusion (CONDITIONAL_SOURCE_GAP, no dedicated appearance-recognition source located).",
      projectArchitectCalibrationQuestions: [
        "Does any worksheet or assessment question show a PHOTOGRAPH (not a schematic symbol) and ask the learner to identify the component?",
      ],
    },
    {
      calibrationKey: "ac6-2-resistor-colour-code",
      acNumber: "6.2", loNumber: 6,
    ...
    // =====================================================================
    // Cross-cutting rows (not tied to one AC row above).
    // =====================================================================
    {
      calibrationKey: "cross-cutting-matrix-vs-baseline-scope-of-analysis",
      acNumber: "1.1", loNumber: 1,
      publicSpecificationAnchor: "N/A -- methodology-scope record, not a content proposition.",
      learnerPerformanceType: "OTHER",
      blindBaselineRequirement: "METHODOLOGY RECORD (corrected by CC-17B -- the previous version of this field hand-counted an intermediate row-mapping breakdown that did not arithmetically reconcile with the ledger's real 60-row total, a pre-existing CC-17 data inconsistency the Project Architect explicitly escalated rather than left for a future session to silently reinterpret): this ledger contains 60 calibration rows; all 56 real CC-16 audit propositions are explicitly mapped to one or more calibrationKeys, or recorded diagnostic-only, through the validator's own CC16_MAPPING table (scripts/content/validate-unit202-blind-calibration-baseline.ts); the validator mechanically proves zero unmapped CC-16 propositions (see its own cc16PropositionsUnmapped report field, and the corresponding regression test). No further hand-maintained intermediate count is stated here, since any such count is derivable, and kept accurate, directly from CC16_MAPPING rather than from hand-authored prose.",
      ...
      blindConfidence: "HIGH",
      matrixComparison: "SAME",
      matrixComparisonNotes: "Not a content comparison -- see rationale.",
      projectArchitectCalibrationQuestions: ["N/A -- this row is a methodology record, not a calibration question."],
    },
  ],
};
