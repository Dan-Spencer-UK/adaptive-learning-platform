...
**Evidence readiness note:** Underlying evidence is PARTIALLY_VERIFIED on BOTH BREADTH and STANDARD CURRENCY, and this learning point must not be published as-is without a Product Architect decision. (1) BREADTH: retrieved symbol evidence covers 13 of the 15 component families. There is NO evidence for the LED symbol -- the standard read does not list a symbol named "light-emitting diode", the nearest being a "photoemissive type" item -- and NO distinct symbol for an INVERTER, which appears only as a lettered general circuit-element rectangle. The LED entry above is therefore ABSENT from the knowledge content deliberately, and must not be filled in from memory. (2) CURRENCY: all symbol evidence traces to the 1975 ANSI Y32.2 / CSA Z99 / IEEE Std 315 standard (self-described as harmonised with IEC Publication 117), read from a third-party university-hosted scan because the IEC 60617 official database and IEC Electropedia both returned 403. NOTHING is verified against the current IEC 60617 database. (3) The resistor has two alternative standard forms (zigzag and rectangle) with no stated preference; the UK convention was not confirmed from a retrieved source. [Section G, third narrow correction pass -- RE-ATTEMPTED, STILL GENUINELY BLOCKED] Freshly re-checked the official current IEC 60617 database via both webstore.iec.ch and library.iec.ch: both independently confirm the same subscription-gated picture (a required License Agreement purchased via IEC National Committees, Sales Outlets, or the IEC Webstore) -- no free per-identity search or geometry view exists for S00684 (LDR)/S00896 (inverter)/S01919 (LED) or any other identity, and no freely-accessible products.iec.ch individual-record route was found. CORRECTION uncovered by this fresh check: BS EN 60617-2:1996 (and the BS EN 60617 series) is WITHDRAWN -- BSI's own current guidance is to use IEC 60617 directly. The prior note's framing ("NOTHING is verified against current BS EN 60617") is corrected: there is no separate current BS EN 60617 edition to check against; IEC 60617 itself is the current standard, and it is this same database that remains paywalled for geometry. This remained the sole genuine core blocker after three successive correction passes' worth of real, bounded, permitted-class-respecting acquisition attempts -- not fabricated closed. Unavailable without payment: symbol GEOMETRY (GIF/EPS/DWG artwork) for all 15 AC6.2 families, and per-entry Standard-vs-Obsolete status, for every identity in this cluster including the six positively identity-confirmed ones (S00641, S00652, S00659, S00684, S01919, S01920). [Section 4/H, this pass -- RESOLVED, per an explicit Product Architect decision] A bounded, genuine re-examination of the SAME already-registered IEEE Std 315-1975 standard (personally re-opened as page images this pass, not from memory or a search snippet) closes both remaining breadth gaps: LED via item 8.5.4.2 'Photoemissive type' (page 56) plus item 11.1.1/NOTE 11.1.1B's 'LED' letter-code association (page 63); inverter via item 16.1.1's own NOTE 16.1C (page 74), which positively confirms the standard's stated policy is a labelled general block precisely when no dedicated symbol exists elsewhere in the standard -- a directly-evidenced, standard-sanctioned absence of a symbol, not merely a failure to find one. Per the Product Architect's decision, the inverter is taught/assessed only as a clearly labelled DC-to-AC functional block, never as a claimed standard component symbol. Breadth is now closed for all 15 named families (14 with a genuinely evidenced dedicated symbol, 1 with a directly-evidenced, standard-sanctioned absence of one); the CURRENCY caveat above remains disclosed but does not block READY, consistent with how it never blocked the other 13 (now 14) families. EDA-LP-16 is now READY; this is no longer a core release blocker.
