...
**Evidence readiness note:** This requirement is VERIFIED and EDA-LP-16 is READY. The 1975 IEEE Std 315-1975/ANSI Y32.2-1975 standard directly and genuinely supports the schematic symbol for fourteen of the fifteen governed component families, including the LED (item 8.5.4.2 'Photoemissive type', cross-referenced to item 11.1.1's explicit 'LED' letter-code association). For the fifteenth family, the inverter, the same standard supports only a labelled general functional block (item 16.1.1, NOTE 16.1C): the standard's own stated policy is that this general block applies precisely where no dedicated symbol appears elsewhere in the standard -- a directly-evidenced, standard-sanctioned absence of a symbol, not an unresolved gap. This evidence traces to the 1975 standard only: current IEC 60617 symbol geometry and per-entry Standard/Obsolete status were not verified (the official database remains subscription-gated and was not reached in any pass). Per an explicit Product Architect decision, this currency limitation is non-blocking at this qualification depth, provided every learner-facing shape is genuinely evidenced or transparently a course-defined functional-block convention. Accordingly, the course must not describe any of these fifteen representations as verified current-IEC-60617 symbols -- only as evidenced against the 1975 standard (fourteen families) or as a labelled functional-block convention (the inverter). Full historical chronology of how this requirement reached VERIFIED, across three prior correction passes, is preserved in the Correction note below and in `UNIT202-CORRECTION-AMENDMENT-LEDGER.json`.

**Correction note:** [Historical chronology, preserved for audit -- superseded by the current-state Evidence readiness note above] Across three correction passes this requirement was recorded PARTIALLY_VERIFIED, held pending a Product Architect decision, for two compounding reasons. (1) BREADTH: retrieved symbol evidence originally covered only 13 of the 15 component families -- no evidence was found for the LED symbol (the standard does not list an entry named "light-emitting diode", the nearest being a "photoemissive type" item) or for a distinct INVERTER symbol (present only as a lettered general circuit-element rectangle). (2) CURRENCY: all symbol evidence traced to the 1975 ANSI Y32.2/CSA Z99/IEEE Std 315 standard, read from a third-party scan because the official IEC 60617 database and IEC Electropedia both returned 403; nothing was verified against the current database. [Section G, third narrow correction pass] A fresh re-check of the official current IEC 60617 database (webstore.iec.ch, library.iec.ch) confirmed the same subscription-gated picture -- no free geometry or per-entry status access exists for any identity in this cluster, including the six identity-confirmed ones (S00641, S00652, S00659, S00684, S01919, S01920). That check also surfaced a genuine correction: BS EN 60617-2:1996 (and the BS EN 60617 series) is WITHDRAWN -- BSI's own current guidance is to use IEC 60617 directly, so there is no separate current BS EN 60617 edition to check against. The requirement remained the sole genuine core blocker after three successive passes' worth of real, bounded, permitted-class-respecting acquisition attempts -- not fabricated closed. [Section 4/H, fourth pass -- RESOLVED, per an explicit Product Architect decision] A bounded, genuine re-examination of the SAME already-registered IEEE Std 315-1975 standard (personally re-opened as page images, not from memory or a search snippet) closed both remaining breadth gaps: LED via item 8.5.4.2 'Photoemissive type' (page 56) plus item 11.1.1/NOTE 11.1.1B's explicit 'LED' letter-code association (page 63); INVERTER via item 16.1.1's own NOTE 16.1C (page 74, read for the first time that pass), which positively confirms the standard's stated policy restricts its general lettered block to situations where no dedicated symbol appears elsewhere in the standard. Breadth closed for all 15 named families; the CURRENCY caveat was retained as disclosed but, per the Product Architect's decision, does not block READY -- consistent with how it never blocked the other 13 (now 14) families. This fifth (current) pass rewrote the Evidence readiness note from a stale PARTIALLY_VERIFIED/must-not-be-published framing with an appended resolution paragraph into the present-state note above, moving this chronology here.

**Learner outcome:** The learner will be able to identify the standard schematic symbol for each Unit 202 component family that has one, and recognise the labelled functional-block representation used for the one family (the inverter) that has no dedicated standard symbol.

**Knowledge / procedure:** Standard graphic symbols for these component families are defined in a recognised graphic-symbols standard. RESISTOR: shown in two alternative forms, a zigzag line and a plain rectangle. THERMISTOR and LDR: the resistor symbol with the appropriate modifier. CAPACITOR: two parallel plates, with the polarised form distinguished. DIODE / RECTIFIER DIODE: a triangle against a bar, between anode (A) and cathode (K). ZENER (breakdown) DIODE: the diode symbol with a bent/flagged bar. PHOTODIODE: the diode symbol with incoming light arrows. LED: the diode symbol with two small arrows directed AWAY from the device, indicating emitted light -- the reverse of the photodiode's incoming-light arrows. TRANSISTOR: the three-terminal symbol with an arrow on the emitter indicating type. SCR: a triangle-and-bar with anode and cathode terminals and a GATE lead entering at the bar. TRIAC: two triangle-and-bar structures in inverse parallel with a single gate lead. DIAC: the bidirectional two-terminal structure with NO gate lead. BRIDGE RECTIFIER: four diode symbols in a diamond bridge. RELAY: a coil rectangle with an associated changeover contact set (a basic form is a circle enclosing the letter R). INVERTER: unlike the other fourteen families, the retrieved standard has no dedicated component symbol for a static inverter -- it is represented only as a labelled general block (the standard's own convention for exactly this situation, per NOTE 16.1C of the retrieved standard). This course accordingly teaches and tests the inverter as a clearly labelled DC-to-AC functional block, never as a standard component symbol in the same sense as the other fourteen families.
...
**Evidence-requirement IDs and their status:**
- `ER::provisional::unit202::electronic-devices-and-applications::schematic-symbol-recognition-for-each-named-ac6-2-component-device-family-at-qualification-depth::SYMBOL_OR_CONVENTION` -- **VERIFIED**
...
- SRC-IEEE315-1975-ANSI-Y32-2-GRAPHIC-SYMBOLS: LED - RESOLVED (Section 4/H, this pass, per an explicit Product Architect decision). Item 8.5.4.2 'Photoemissive type', cross-referenced to item 11.1.1's 'LED' letter-code...
- SRC-IEEE315-1975-ANSI-Y32-2-GRAPHIC-SYMBOLS: INVERTER - RESOLVED as a confirmed, standard-sanctioned absence of a dedicated symbol (Section 4/H, this pass). Item 16.1.1 'General' plus NOTE 16.1C...
...
- SRC-IEC-60617-WEBSTORE-PREVIEW: The official IEC 60617 webstore free preview directly confirms six current symbol identity numbers and their English names, all genuinely read -- not the basis for this requirement's VERIFIED status (see EVIDENCE-RESULTS.json).
...
- Identify the LED symbol (the diode symbol with two outgoing light arrows) on a circuit diagram.
- Explain why the inverter is shown on a diagram as a clearly labelled DC-to-AC functional block rather than a standard component symbol like the other fourteen families.
...
**Explicit exclusions:**
- The internal circuit of any specific alarm (see EDA-LP-25, which is READY on a narrower, representative-exemplar scope)
...
### `EDA-LP-25` -- Security alarm application: SCR latching keeps a sounder energised (representative exemplar) [TRANSFORMED_TO_EXEMPLAR]

**Evidence readiness:** READY

**Outstanding production dependency:** (none -- REPRESENTATIVE_EXEMPLAR_AUTHORING is resolved by this pass's PA decision, see below)

**Correction note (false-green re-adjudication):** Previously marked READY on the strength of general BJT switching behaviour alone, which was a false green: this learning point's own evidence requirement asks specifically about the transistor's role WITHIN a security-alarm circuit, and no read source ties the general cutoff/saturation behaviour to any alarm application. Reverted to HELD_PENDING_EVIDENCE_CORRECTION. [Section F, third narrow correction pass -- RESOLVED] The alarm-specific sounder-role required facet is now VERIFIED via a genuine, narrowly-scoped Product Architect decision (PA-UNIT202-20260906-SCR-ALARM-EXEMPLAR-001, see UNIT202-PA-AUTHORITY-DECISION-REGISTRY.json) authorizing Ray Marston's authored Nuts & Volts articles as AUTHORITATIVE_TECHNICAL_REFERENCE for this one bounded relationship. The retired exact NC-loop/bias topology remains retired and is NOT restored; no specific circuit's component values are taught. EDA-LP-25 moves to READY, MIXED_REQUIRED_AND_CONTEXT (required: SCR-sounder relationship, now VERIFIED; context: transistor-switching-in-alarm-context and the retired topology, both still correctly excluded from required recall).

**Evidence readiness note:** [Section F, third narrow correction pass -- RESOLVED] `security-alarm-scr-thyristor-latching-sounder-role` is now VERIFIED: the SCR-latching mechanism itself was already fully evidenced (Littelfuse AN1002, manufacturer, unchanged); the previously-unevidenced sounder-role half is now evidenced via a genuine Product Architect decision (PA-UNIT202-20260906-SCR-ALARM-EXEMPLAR-001) authorizing Ray Marston's authored Nuts & Volts articles ("SCR Principles and Circuits", Jan 2002; "Security Electronics Systems and Circuits" Part 4, 1998 -- authorship independently confirmed via Marston's own contributor page) as AUTHORITATIVE_TECHNICAL_REFERENCE for this one bounded relationship only. `security-alarm-exact-nc-contact-bias-topology` remains RETIRED_OUT_OF_SCOPE (unaffected -- no specification exists to evidence, and this decision does not restore it). `security-alarm-transistor-switching` remains PARTIALLY_VERIFIED and CONTEXTUAL (the general transistor cutoff/saturation behaviour is separately READY under EDA-LP-10; no source ties it specifically to an alarm circuit, and none is required to, since it is not this LP's required facet). EDA-LP-25 is READY on its required facet. [Section 3A, this pass -- current-state consistency correction] assessmentEligibility corrected from the contradictory stale BLOCKED to ASSESSABLE_WHEN_READY, matching a READY LP with a VERIFIED required facet. Title, learnerOutcome, knowledgeOrProcedure and contextualExamples narrowed to describe only the genuinely READY scope: the SCR/sounder relationship (required) and, as pure background, generic transistor switching (contextual, taught in full under EDA-LP-10).

**Learner outcome:** The learner will be able to explain the general, representative relationship whereby a triggered, latched SCR can keep an alarm sounder (bell/buzzer) energised until reset, without reference to any one specific circuit's exact topology.

**Knowledge / procedure:** SCR LATCHING (required mastery): once a momentary gate pulse has triggered an SCR and the anode current exceeds the latching current, it remains conducting even after the trigger is removed, until the current falls below the holding current. ALARM APPLICATION (required mastery, representative exemplar only): a triggered, latched SCR can be used to keep current flowing through an alarm bell, buzzer or other sounder, remaining conducting until the current is reduced below holding current -- commonly by interrupting the supply or operating a manual reset switch. This is a representative introductory relationship, not a specific compliant security-installation design: no exact normally-closed loop, bias topology, component values, or SCR-to-sounder wiring diagram is taught as canonical mastery, and no claim is made about how the sounder behaves if a loop is reclosed. CONTEXT ONLY, not required recall: a bipolar transistor used as a switch operates as a non-linear switch alternating between the CUTOFF region (an open switch, both junctions reverse biased) and the SATURATION region (a closed switch) -- this generic switching behaviour is its own required mastery under EDA-LP-10, and no source here ties it to a role within any specific security-alarm circuit or normally-closed detection loop.
...
- `ER::provisional::unit202::electronic-devices-and-applications::security-alarm-scr-thyristor-latching-sounder-role::EXACT_FACT` -- **VERIFIED**
- `ER::provisional::unit202::electronic-devices-and-applications::security-alarm-transistor-switching::EXACT_FACT` -- **PARTIALLY_VERIFIED** (general BJT cutoff/saturation switching is fully evidenced as a separate technical fact under EDA-LP-10, but no source ties it to a security-alarm circuit specifically; this is a contextual, not required, facet)
...
- SRC-NUTSVOLTS-MARSTON-SCR-PRINCIPLES: A triggered, latched SCR can be used in an alarm circuit to keep current flowing through a bell, buzzer or other self-interrupting sounder load, remaining conducting until the anode current is reduced below holding current...

**Depth justification:** The three circuit-level alarm requirements are combined into one learning point because all three presuppose the SAME unidentified circuit. The transferable device-level roles (transistor switching, SCR latching) are genuine, separately-evidenced technical truth and are independently READY as their own prerequisite learning points (EDA-LP-10, EDA-LP-11); this learning point's own required facet is the SCR-sounder relationship, now VERIFIED. Kept separate from EDA-LP-24 so the application-category content remains independently deliverable.

**Explicit exclusions:**
- The exact retired NC-detector/bias topology (see `security-alarm-exact-nc-contact-bias-topology`, RETIRED_OUT_OF_SCOPE) -- not restored by this resolution
- Any specific circuit's exact component values (resistor values, specific part numbers) -- context only, not required recall
- Presenting the taught relationship as a compliant, installable security-alarm design -- it is a representative introductory exemplar only
- End-of-line resistor supervision and tamper-loop design
- Alarm panel programming and installation practice

**Representative application types:**
- Explain, in general terms, why an SCR's latching property makes it a plausible building block for keeping an alarm sounder (bell/buzzer) energised once triggered, and how the circuit can be reset.
...
### `EDA-LP-28` -- Inside the UK master socket: ringer capacitor, 470 kΩ resistor and surge protector

**Evidence readiness:** READY

**Evidence readiness note:** [Resolved, Stage D bounded acquisition, narrow correction pass] The required capacitor-to-ringer functional mechanism is now VERIFIED via a disclosed, transparent two-source composition: Openreach SIN 351 (UK-specific topology/presence/connection/values) plus IXYS/Littelfuse AN-144 (the general historical telephone-ringer DC-blocking mechanism this exact topology represents -- not claimed to describe the Openreach NTE specifically). The contextual 470 kilohm resistor line-testing attribution remains genuinely unevidenced and stays correctly excluded from required/taught content.
...
**Knowledge / procedure:** Openreach specifies that the master socket contains, connected ACROSS the "A" and "B" wires: a 1.8 MICROFARAD CAPACITOR in series with a 470 KILOHM RESISTOR, with the socket's SHUNT CONNECTION (named the 'Bell wire') taken from the CENTRE POINT between the two; and, separately, an OVER-VOLTAGE PROTECTION DEVICE connected across the "A" and "B" wires. CAPACITOR FUNCTION (required): in the general topology this arrangement represents -- a capacitor in series with an inductive ringer -- the capacitor's role is to DC-block: it prevents the line's d.c. condition from passing to the ringer while allowing the a.c. ringing signal through, with the capacitor's value chosen for resonance near the ringing frequency. RINGING (context only, not required recall): UK call-arrival indication is a 25 Hz alternating signal of 40-100 V a.c. r.m.s. applied across the "A" and "B" wires. LINE TESTING (context only, not required recall): Openreach runs periodic automatic test routines on its local line plant; no authoritative source states the 470 kilohm resistor's function as a line-test resistor specifically, and that attribution remains unevidenced and must not be taught as established. SURGE PROTECTION: the analogue line is subject to noise, induced voltages and line surges, which is what the over-voltage protection device guards against. JURISDICTION AND ERA: UK legacy analogue PSTN only.
