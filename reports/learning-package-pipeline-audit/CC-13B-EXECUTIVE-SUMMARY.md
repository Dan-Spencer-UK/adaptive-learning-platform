...
- **Missing component symbol imagery**: reproducible; the deterministic symbol cards are all shipped correctly, but their paired physical-recognition companion photos were produced, QA-passed, and simply never integrated — one of the cheapest gaps in this whole audit to close, since no new production is needed, but (corrected by CC-13B.2) the old QA-`PASS` status is reusable evidence, not itself authorisation to ship; each asset must still be qualified through the current production-eligibility authority (`REMEDIATION-PLAN.md` Package 6) before integration.
...
Full detail, dependencies, and acceptance criteria: `REMEDIATION-PLAN.md`. **Corrected by CC-13B.1 and CC-13B.2**: the original CC-13B plan omitted a package for one of this same audit's own confirmed P0 findings (no schema field for extended teaching prose) — closed by inserting Package 2 (CC-13B.1). It also sequenced the visual-governance packages so that legacy-pipeline assets could have become learner-visible before passing the new ADR-0005 reference-authority and production-eligibility chain, and implied a historical reference handover could automatically count as an approved `ReferenceDossier` — corrected (CC-13B.2) by resequencing to visual planning mandatory (3) → reference-dossier authority (4) → canonical production-eligibility authority + release-ID fix (5) → qualify/integrate reusable legacy assets (6), with an explicit rule that legacy QA/reference evidence is reusable but never automatically authorising. Summary order: (1) close the `branchRoutes` gap, (2) extend the canonical teaching-content model for rich scrollable teaching, (3) make visual planning mandatory, (4) formalise reference authority as real, Project-Architect-reviewed `ReferenceDossier` records, (5) establish one canonical production-eligibility authority and close the release-ID drift, (6) qualify/integrate reusable legacy visual assets, (7) implement the formative/mock assessment feature, (8) wire Guided Revision (depends on 7), (9) wire publication gates into real release eligibility (depends on 2, 3, 4, 5, 7, 8), (10) remove the redundant authored `exit_completion` step from the remaining 23 lessons, (11) V1/adaptive-engine labelling cleanup, (12) debug-leakage defense-in-depth generalisation, (13) — **only after the V1 pilot passes** — systematically rebuild Unit 202 through the corrected pipeline.

## Recommendation: SAFE TO PROCEED TO REMEDIATION (not to full Unit 202 re-authoring)

This audit's own findings, not merely its own completion, support this conclusion: the repository has a sound, real, well-tested schema/validator foundation (CC-13A's work holds up under direct scrutiny — every claim independently checked in this audit was accurate except one narrow overstatement, MM-1), a genuinely working curriculum/lesson/runtime pipeline through lesson delivery (CC-12H remains valid), and a clearly-enumerated, evidence-backed set of gaps with a concrete, dependency-ordered remediation plan. **It is safe to begin Packages 1-12 of the remediation plan, pending Product Owner / Project Architect review and sequencing.** It is explicitly **not** yet safe or appropriate to begin systematic Unit 202 re-authoring (Package 13) — that step is gated on the V1 pilot passing, which itself depends on Packages 2, 3, 4, the Part-A/Part-C portion of 5, 7, 8 and 9 existing first (Package 6 and Package 5 Part B are not themselves pilot prerequisites unless the pilot deliberately reuses a legacy visual asset). Qualification of the pipeline is a separate, future determination this audit does not make; this audit establishes the honest starting point for it.
