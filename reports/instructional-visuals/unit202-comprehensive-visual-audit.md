...
**7.4 Art-job accounting now means real manual work.** `dashboard.ts`'s `requiredPremiumHybridArtJobs`/`usefulPremiumHybridArtJobs` count only promptable, non-deterministic assets — i.e. the actual number of separate ChatGPT sessions the Product Owner would need to run, not an abstract asset count. At the end of CC-11.7A: **25 REQUIRED art jobs**, **0 USEFUL art jobs live** (all 8 non-deterministic USEFUL findings are honestly blocked pending a reference — once one is sourced and marked READY, it becomes a real, separately-trackable art job, never silently folded into an existing prompt).

---

## 8. CC-11.7B addendum — shared-base validation, corrected required/blocked accounting, and the final review pack

CC-11.7B is the FINAL pre-production audit: it asks whether the 98 canonical states CC-11.7/CC-11.7A produced were compressed into the *correct* number of actual production images, corrects a real accounting bug, and produces the Product Owner's pre-production PDF review pack.

**8.1 Every multi-state ProductionAsset audited for genuine shareability.** All 18 multi-state assets (at the CC-11.7A baseline) were individually reviewed against three questions: does the base artwork's own text already ask for more than one image (`exactDeliverable` audit); do the grouped states differ only by a deterministic overlay (label, arrow, dimension, reveal/hide), or by a genuine pose/configuration/scene change; and does a declared `deterministicOverlayResponsibilities` entry actually cover the difference. Two real defects were found in `exactDeliverable` text alone (`unit202.magnet.poles` and `unit202.diode.bias-direction` both literally asked for "Two premium illustrations" inside one prompt) — the exact composite-violation pattern task brief CC-11.7A §8 already named as unacceptable, that CC-11.7A's own split (of `components.physical`) had not caught because it only affected `PHYSICAL_RECOGNITION`-role assets.

**8.2 Five assets split, one asset's internal accounting corrected, one role corrected.**
- `unit202.motor.effect` (8 states) → split by pole orientation (a genuine apparatus-layout change) into `.horizontal-poles` / `.vertical-poles` (4 states each); current direction within one orientation remains a shared deterministic-overlay concern.
- `unit202.generator.rotating-loop` (2 states) → split by loop pose (face-on vs edge-on is a materially different 3D silhouette, not an overlay difference) into `.horizontal` / `.vertical` (1 state each).
- `unit202.magnet.poles` (4 states, and the literal "Two premium illustrations" defect) → split by pole pairing (attracting vs repelling magnets are conventionally composed with different spacing/field behaviour) into `.like` / `.unlike` (2 states each).
- `unit202.gears` (3 states) → split by size ratio (a genuine physical size change to a rendered object, with no overlay mechanism able to resize a gear) into `.driven-larger` / `.driven-smaller` / `.equal` (1 state each).
- `unit202.diode.bias-direction` (2 states, and the literal "Two premium illustrations" defect) → split by bias direction (conducting vs blocked is a genuine presence/absence scene difference) into `.forward` / `.reverse` (1 state each).
- `unit202.current-conductor.magnetic-field` (4 states) → **kept, with conditions**: current-direction/field-circulation indicators were incorrectly described as baked-in artwork; corrected to explicit deterministic-overlay responsibilities so one direction-neutral base image safely serves both current directions (the same reasoning the right-hand-grip mnemonic asset already used).
- `unit202.protective-devices` — role corrected from `PHYSICAL_RECOGNITION` (a mismatch for a two-object comparison) to `COMPARISON`; explicitly confirmed as one intentional fuse-vs-breaker composite image, matching task brief §8's own "ACCEPTABLE" example.

Zero states lost: 98 before, 98 after (all states redistributed onto the split assets' new homes, never dropped). Zero historical-66 variants disturbed (all pre-existing `existingCanonicalVariantId` values carried over unchanged onto their states' new parent assets, re-verified by the same mechanical reconciliation check).

**8.3 REQUIRED/USEFUL vs BLOCKED_REFERENCE conflation fixed.** `visualNeedClassificationFor()` previously returned `"BLOCKED_REFERENCE"` for any asset with `referenceReadiness: "NOT_READY"`, silently overriding its REQUIRED/USEFUL classification and excluding it from the REQUIRED/USEFUL totals in every downstream count. Corrected: pedagogical need (REQUIRED/USEFUL/DEFERRED_SCOPE, via `visualNeedClassificationFor()`) and production readiness (READY/blocked, via the new `isReferenceBlocked()`) are now two fully independent dimensions everywhere in the Studio — a REQUIRED-but-blocked asset is REQUIRED, not silently reclassified. `dashboard.ts` now reports `requiredTotal`/`requiredReady`/`requiredBlocked` and `usefulTotal`/`usefulReady`/`usefulBlocked` as six independent numbers, plus a `requiredVisualProductionComplete` boolean that is true only when every REQUIRED art job is approved AND `requiredBlocked` is exactly 0.

**8.4 Final review pack.** `tools/visual-production-studio/generate-final-review.ts` (`npm run visuals:studio:final-review`) generates `reports/instructional-visuals/unit202-final-visual-production-review.{pdf,json}` directly from the live catalogue/dashboard/audit data — a Product-Owner-uploadable, repository-independent pre-production review document (cover, executive summary, full catalogue by LO with real embedded deterministic previews where they exist and honest "ARTWORK NOT YET PRODUCED" placeholders where they don't, multi-state sharing review table, the actual 42-row image-generation job list, deterministic visual list, blocked-references section, directional-safety section, component-recognition section, final production-readiness table, final mechanical audit statement).

**8.5 Updated final counts.** Visual families: **21** (unchanged). Production/base assets: 47 → **53** (+6 from the 5 splits, net). Canonical states: **98** (unchanged — zero lost). REQUIRED: 33 → **42** total (39 ready, 3 blocked — up from 33 purely because BLOCKED_REFERENCE assets are no longer excluded from the count, not because new REQUIRED needs were found). USEFUL: **10** total (2 ready, 8 blocked — same correction). Deterministic-only: 11. DEFERRED_SCOPE: 1. Art jobs: **42 total** (34 REQUIRED — 31 ready, 3 blocked; 8 USEFUL — 0 ready, 8 blocked). `REQUIRED VISUAL PRODUCTION COMPLETE`: **false** (0 of 34 REQUIRED art jobs approved, and 3 remain blocked).
