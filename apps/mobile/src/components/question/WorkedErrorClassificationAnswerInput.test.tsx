...
describe("WorkedErrorClassificationAnswerInput", () => {
  it("renders the shown-working lines it is given", async () => {
    const { getByText } = await render(
      <WorkedErrorClassificationAnswerInput shownWorkingLines={["I = V x R", "I = 12 x 4 = 48 A"]} onSubmit={jest.fn()} />,
    );
  ...
  it("calls onSubmit with the classification value for the tapped option", async () => {
    ...
    const { getByLabelText } = await render(<WorkedErrorClassificationAnswerInput shownWorkingLines={["I = V x R"]} onSubmit={onSubmit} />);
  ...
  it("does not call onSubmit when disabled", async () => {
    ...
    const { getByLabelText } = await render(
      <WorkedErrorClassificationAnswerInput shownWorkingLines={["I = V x R"]} onSubmit={onSubmit} disabled />,
    );
