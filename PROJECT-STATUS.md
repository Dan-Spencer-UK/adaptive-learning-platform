...
## CC-06B — Deterministic Learner-Specific Lesson Assembly Engine (implementation-complete, review-ready)
...
- **Deterministic identity** (`packages/learning-engine/src/identity.ts`): same lesson id/version/content release/assembly-policy version/evidence digest ⇒ same `instanceId`, independent of Map/Set/object key ordering (FNV-1a, mirrors `packages/calculation-engine/src/seed.ts`'s Hermes-portable pattern — no Node `crypto`, safe for on-device use).
...
- 51 new package tests + 2 new gate tests, full root Vitest suite 437/437, `lesson:validate:check`/`lesson:assembly:prove:check`/`content:pedagogy:check` all green, typecheck/lint clean across the whole repo.
