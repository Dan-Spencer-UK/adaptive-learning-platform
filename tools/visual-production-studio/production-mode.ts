...
export const NO_GENERATIVE_INFERENCE_RELATIONSHIPS: readonly string[] = [
  ...
  "B / l / v geometry for motional EMF: l is the active conductor length, v (velocity) is perpendicular to l, B (field) is perpendicular to both l and v (CC-11.13A: named explicitly after `unit202.emf.motional`'s l-dimension defect)",
...
export const PRODUCTION_MODE: Record<string, ProductionModeRecord> = {
  ...
  "unit202.components.physical.capacitor": mode(
    "ORIGINAL_REDRAW_FROM_REFERENCE",
    "Physical form grounded on a real reference photo. CC-11.13A correction: CC-11.12 removed this asset's baked UK/IEC symbol 'for family consistency' -- that was the WRONG direction. The corrected component-family rule (see §11 platform note): PHYSICAL COMPONENT RECOGNITION IMAGE + DETERMINISTIC UK/IEC SYMBOL COMPANION, for every component, capacitor included -- the fix should have added a deterministic symbol companion to the other components, never removed the capacitor's. Not corrected now (Unit 202 component cleanup remains deferred); recorded so future course production implements the rule correctly from the start. See asset-lifecycle.ts's DEFERRED_SCOPE entry for this asset.",
  ),
