...
export const ROADMAP: RoadmapItem[] = [
  ...
  },
  {
    ...
    notes:
      "Product Owner / ChatGPT completed semantic review of all 51 generative outputs -- a review of whether the actual reference frame supplied to Gemini was semantically fit for the exact governed state, not geometry alone. Returned a PERMANENT product/content-governance decision (all future modules/courses/qualifications): reference-first is necessary but not sufficient. Recorded in the style guide (new SS9), pipeline doc (new SS22), ADR-0004, and architecture overview. Added a minimal semantic-QA data model (semantic-reference-qa.ts, 51 records) and a hard gate in run-production.ts. 51/51 records matched the live catalogue. 29 KEEP entries left untouched; 22 REDO entries remediated exactly as specified -- right-hand-grip, the full 8-state motor-effect family (rebuilt from hand-authored exact-state reference boards), the AC generator pair (replaced a never-actually-visible reference with a real public-domain source), Class III lever (excavator contamination removed), like/unlike magnets (answer text removed), resistivity (course-conflicting S notation fixed), motional EMF (photoreal 3D replaced with minimal 2D), physical capacitor (baked symbol removed), diode-bias pair (rebuilt as a matched schematic circuit pair), electrolysis (a wrong cation arrow a prior automated PASS missed is now fixed and re-verified), conductor-vs-insulator (replaced two near-identical photos with one labelled cutaway). Replaced the single-verdict audit with three independent verdicts (TECHNICAL / PEDAGOGICAL_CLARITY / VISUAL_PRODUCT_QUALITY) -- 22/22 REDO entries PASS on all three, 0 HUMAN_REVIEW_REQUIRED. New remediation review pack shows OLD/NEW reference/output and all three verdicts for every REDO entry, 0 missing-reference hard failures. Full production review regenerated: 51 attempted, 51 PASS, 92 total Gemini calls.",
    gate: "UNIT 202 GENERATIVE VISUAL REMEDIATION COMPLETE -- AWAITING PRODUCT OWNER / CHATGPT REVIEW. See reports/instructional-visuals/unit202-semantic-qa-remediation-review.pdf and unit202-visual-production-review.pdf.",
  },
  { id: "cc-12", label: "CC-12 -- Electrical Power Slice", kind: "STAGE", status: "NOT_STARTED", dependsOn: ["cc-11.12"], notes: "Explicitly not started -- next course after Unit 202. Blocked on: Product Owner / ChatGPT review of the semantic-QA-remediated Unit 202 visual package." },
