...
export const ROADMAP: RoadmapItem[] = [
  ...
  { id: "cc-11.7c", label: "CC-11.7C -- Tighten Visual Production Readiness", kind: "STAGE", status: "COMPLETE", dependsOn: ["cc-11.7b"], evidence: "Commit 34318a2 -- corrected reference-readiness for 15 assets, hand-rule mirror prohibition, deterministic-technical assets excluded from promptable/art-job flow." },
  {
    ...
    notes:
      "Establishes the canonical Claude-orchestrates / Gemini-renders / ChatGPT-researches-references division of responsibility, the ALP instructional visual style guide, and proves the pipeline end-to-end on 2 real assets via the live Gemini API. Also builds this dashboard. Does not process the full Unit 202 image queue and does not start CC-12.",
    gate: "Product Owner (and ChatGPT independent review) must review reports/instructional-visuals/unit202-visual-production-proof-review.pdf before any wider automated production run.",
  },
  { id: "cc-12", label: "CC-12 -- Electrical Power Slice", kind: "STAGE", status: "NOT_STARTED", dependsOn: ["cc-11.8"], notes: "Explicitly not started -- next course after Unit 202. Blocked on: CC-11.8's proof-review gate, then the remaining 3 REQUIRED + 8 USEFUL blocked-reference assets and the full REQUIRED art-job production run." },
