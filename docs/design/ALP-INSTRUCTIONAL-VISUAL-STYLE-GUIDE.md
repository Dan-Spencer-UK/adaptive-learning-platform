...
**Relationship to the product-wide design system (CC-13A, 2026-08-29):** [`ALP-PRODUCT-WIDE-VISUAL-DESIGN-SYSTEM.md`](ALP-PRODUCT-WIDE-VISUAL-DESIGN-SYSTEM.md) is now the single product-wide authority for line/stroke/palette/symbol/layout rules across every qualification, present and future. This document is retained as the **generated-art production subset**: the Gemini-specific generation mechanics (raster-vs-overlay responsibility, master/asset-specific prompt layering, reference-vs-style-reference distinction, the semantic reference QA gate, production-mode/lifecycle mechanics) and the CC-11.x decision history behind them. Where a value could be read as a competing authority (background hex values, colour roles, family-consistency rules), the product-wide system's values are the ones both documents state — this document does not restate them as an independent decision. There are not two competing style authorities: for any product-wide visual rule not specific to the generation mechanics below, defer to `ALP-PRODUCT-WIDE-VISUAL-DESIGN-SYSTEM.md`.
...
**CORRECTED 2026-08-26 (CC-11.13, Product Owner decision):** "tactile -- real materials, real surfaces, real light" above describes the finish quality bar (premium, not clip-art), not a mandate to render every asset as a photoreal 3D object. Whether an asset is 3D/tactile or clean 2D/diagrammatic is governed separately by §10 below -- physical-component recognition assets (a resistor, a capacitor) benefit from tactile realism because the physical form IS the fact being taught; most governed *teaching diagrams* (phenomena, mnemonics, comparisons, circuits) do not, and default to 2D per §10.
...
## 10. Reference-as-authority, not reference-as-final-asset — and the 2D-diagrammatic default

**Recorded 2026-08-26 (CC-11.13 — Product Owner decision, permanent, applies to all future modules/courses/qualifications).** Full decision record: `docs/architecture/PREMIUM-INSTRUCTIONAL-VISUAL-PRODUCTION-PIPELINE.md` §23, ADR-0004's Consequences, `tools/visual-production-studio/production-mode.ts`.

**Simply vectorising or tracing a reference image is not an acceptable final production method.** §5 already establishes that a technical reference is the authority for geometry/direction/topology; CC-11.13 makes explicit what "authority" means for production: where a reference is clear and technically suitable, it is used as the technical authority and **redrawn as an original ALP-styled teaching diagram**, with **no artistic licence over the technical relationships it establishes**. The reference is never lifted, traced, or reproduced as the shipped asset -- and the redraw is never a free re-interpretation that drifts from what the reference actually shows.

**No artistic licence over technical relationships.** A redraw may change material, finish, lighting, composition and every other purely stylistic quality; it must never change: direction, handedness, pole arrangement, conductor topology, lever-class relationship, diode polarity/current meaning, B/l/v geometry, ion-movement direction, circuit topology, or any other governed technical fact the reference establishes. See `tools/visual-production-studio/production-mode.ts`'s `NO_GENERATIVE_INFERENCE_RELATIONSHIPS` for the full platform-wide list of relationships that must never be left to generative inference, under any production mode.

**2D/diagrammatic is the default, not 3D.** These are teaching diagrams, not product-shot renders. Do not bias the system toward 3D by default -- in most cases a clean 2D/flat/diagrammatic treatment communicates the concept more clearly than a perspective render, and §1's "tactile, real materials, real light" language (product finish quality) must never be read as a mandate for 3D/photoreal treatment. Perspective/depth/3D rendering is **opt-in only**, used when it materially improves comprehension of a genuinely spatial relationship (e.g. the AC generator's loop/shaft/slip-ring topology, where a flat projection would itself be ambiguous) -- never applied uniformly as a default "premium" aesthetic. `unit202.emf.motional`'s CC-11.12 correction (a photoreal 3D pipe-apparatus treatment replaced with a minimal flat 2D board) is the direct evidence motivating this default.

**Governed production modes.** Every asset's actual production method is recorded in `tools/visual-production-studio/production-mode.ts` as one of five modes -- `DETERMINISTIC_TECHNICAL`, `ORIGINAL_REDRAW_FROM_REFERENCE` (the default for most teaching diagrams), `HYBRID_REDRAW_PLUS_DETERMINISTIC_OVERLAY`, `GENERATIVE_CONCEPTUAL`, and `DEFERRED_POLISH` (a status, not a creation method). Generation via an image model is a production **option** selected per asset, never a mandatory step for every asset -- see the pipeline doc §23 for the full model and the asset-lifecycle gating this feeds into (`tools/visual-production-studio/asset-lifecycle.ts`).
