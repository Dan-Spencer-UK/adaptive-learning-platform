...
## 11. Learning-package integration requirements (ADR-0005/ADR-0006, CC-13A)

Following Product Owner review of Unit 202 (2026-08-29; see [`ADR-0005`](../architecture/adr/ADR-0005-learning-package-production-and-visual-governance.md), [`ADR-0006`](../architecture/adr/ADR-0006-v1-canonical-lessons-and-assessment-driven-guided-revision.md)), the following are acceptance criteria for this standard, not aspirational:

- **No internal/debug metadata in production learner views.** Step ids, engine field names, raw numerical tolerance metadata, branch-trigger names or any other authoring/engine-internal value must never render in a learner-facing screen — a direct finding of the Unit 202 review. See [`docs/architecture/LESSON-PLAYER-AND-LESSON-PLAN-ARCHITECTURE.md`](../architecture/LESSON-PLAYER-AND-LESSON-PLAN-ARCHITECTURE.md) §18.2.
- **The teaching-to-check transition must prevent answer leakage.** Teaching content must not remain visible/answer-bearing once an embedded formative check targeting the same capability is active — see `LESSON-PLAYER-AND-LESSON-PLAN-ARCHITECTURE.md` §17 for the governing reveal rule this UI must enforce.
- **The illustrated visual canvas may legitimately differ from the app's dark chrome.** A white/near-white illustrated-visual background (per [`docs/design/ALP-PRODUCT-WIDE-VISUAL-DESIGN-SYSTEM.md`](../design/ALP-PRODUCT-WIDE-VISUAL-DESIGN-SYSTEM.md) §2) is not a design-system violation merely because it differs from the app's dark surface tokens — the two are governed separately and intentionally.
- **The scroll-discoverability requirement remains in force**, owned by `LESSON-PLAYER-AND-LESSON-PLAN-ARCHITECTURE.md` §4 (not duplicated here): where a step's content extends below the fold, the interface must make that unambiguous.
- **Do not optimise teaching layout to fit one viewport.** Per ADR-0006, rich scrollable teaching is preferred over fragmenting one coherent explanation to avoid scrolling; native UI implementation must not silently reintroduce a one-viewport-per-step constraint for layout convenience.
- **Component/symbol visuals and interactive visual regions use accessible mobile touch targets**, per §7/§8 above — a diagram's interactive hit regions are held to the same minimum touch-target sizing as any other control.
- **Guided Revision has a clear learner-facing entry point and ordered lesson list.** The V1 Guided Revision plan (full canonical lessons ranked by weakness, [`docs/architecture/V1-LEARNING-ASSESSMENT-AND-GUIDED-REVISION-ARCHITECTURE.md`](../architecture/V1-LEARNING-ASSESSMENT-AND-GUIDED-REVISION-ARCHITECTURE.md)) must be reachable through an obvious, discoverable native entry point, presented as an ordered list a learner can act on directly.
- **Assessment submission is explicit.** A dedicated formative/mock assessment's completion must include an unambiguous, explicit submission action — the UI must never treat merely finishing the last question, backgrounding the app, or navigating away as an implicit submission, since only an explicit `SUBMITTED` state may update Guided Revision.
