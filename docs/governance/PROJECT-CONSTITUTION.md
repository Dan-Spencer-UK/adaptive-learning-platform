...
Governed knowledge is normally **domain knowledge, not course-owned knowledge**: the same fact, relationship, mechanism, symbol meaning or calculation rule may legitimately be required by multiple units within one qualification, multiple qualifications from the same awarding body, qualifications at different levels, or potentially other awarding bodies. Knowledge must not be duplicated merely because different courses or units require it. Course/qualification/unit mappings and a package's own learning-purpose/depth definition determine whether knowledge is required, which subset is required, how deeply the learner must work with it, and what performance is expected — they do not normally redefine the underlying fact. Where a higher-level qualification requires additional depth, existing valid knowledge should be reused and only the genuinely additional governed knowledge/relationships added, rather than cloning a lower-level corpus. Course-specific knowledge is valid only where the subject matter itself is genuinely specific to that qualification/context/jurisdiction. Conceptually: authoritative technical source → reusable governed domain knowledge → course/qualification/unit mappings → package-specific learning purpose/depth → required learner performance → lesson/assessment. This document does not fix an identifier strategy or database schema for that reuse.
...
**Source-role separation (2026-08-30):** curriculum authority (what is in scope), depth/performance evidence (how much a learner must understand or be able to do), and technical/factual authority (what is actually true) are three distinct evidential roles and must never be conflated — a source that legitimately establishes one does not thereby establish another. Full application to City & Guilds packages: [`docs/architecture/SYLLABUS-TO-LEARNING-PACKAGE-PRODUCTION-ARCHITECTURE.md`](../architecture/SYLLABUS-TO-LEARNING-PACKAGE-PRODUCTION-ARCHITECTURE.md) §3.4.

**LLMs are never sources of truth (2026-08-30):** an LLM may extract, classify, compare, structure, propose searches, locate candidate references, draft explanations from governed evidence, and identify possible gaps for human/independent review — but an LLM's internal knowledge is never acceptable provenance for a governed technical/factual knowledge claim. Every governed factual/technical knowledge item that requires external factual authority must trace to suitable authoritative source evidence; if sufficient evidence cannot be found, the item remains unresolved/unsupported rather than filled from an LLM's memory.
