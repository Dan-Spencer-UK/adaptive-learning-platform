...
Thirteen gap types, each preserving the affected candidate key, evidence already available, exactly what is unresolved, and a **plural** `legitimateResolverRoles`:
...
**CC-20 restates the full production sequence** this package implements, now with semantic adjudication and depth/knowledge-boundary status as explicit stages rather than left implicit inside source normalization or technical-source matching:
...
RAW EVIDENCE
  ...
  -> TECHNICAL-TRUTH RESOLUTION       (attachFactualClaims, detectFactualConflicts -- claim-key-exact
                                         coverage/conflict detection, §16.2-16.3; TECHNICAL_TRUTH never
                                         creates scope, §2)
  -> SEMANTIC ADJUDICATION            (validateSemanticAdjudications, detectSemanticAdjudicationConflicts --
                                         a governed DECISION over already-existing proposals, §21; evidence-
                                         bound, but never itself a source authority, §21.5)
  -> GOVERNED KNOWLEDGE BOUNDARY      (finalizeKnowledgeBoundary -- knowledgeBoundaryStatus,
                                         corrected technicalCoverageStatus, KNOWLEDGE_DECOMPOSITION_GAP, §22)
  -> DEPTH STATUS                     (attachQualificationLevelConstraints, computePerformanceDepthGaps --
                                         depthBasis, assessmentCalibrationAvailable, §23)
  -> COURSE-CONSTRUCTION HANDOFF      (StandardPipelineResult -- see below)
...
This package produces `StandardPipelineResult` (`candidates`, `gaps`, `unmatchedTechnicalTruth`, `unmatchedQualificationLevel`, and — CC-20 — `semanticAdjudicationOutcomes`, `representativeExemplars`) and stops. Downstream course construction can distinguish, without inferring anything from prose: explicit required curriculum scope (`candidates` filtered by `disposition`); governing vs. unresolved learner knowledge (`requiredFactKeys` + `knowledgeBoundaryStatus`); accepted representative exemplars (`representativeExemplars`, never present in any candidate's `requiredFactKeys`); contextual/rejected knowledge (`semanticAdjudicationOutcomes` filtered by `decision`); qualification-level-bounded vs. assessment-calibrated depth (`depthBasis`); technical coverage (`technicalCoverageStatus`); and unresolved semantic decisions (`gaps` filtered by `SEMANTIC_ADJUDICATION_GAP`/`SEMANTIC_ADJUDICATION_CONFLICT`).
...
- **`GOVERNED`** — the current learner-knowledge boundary is sufficiently represented by governing fact/procedure requirements (`technicalCoverageStatus: COMPLETE`, no pending `REVIEW_PROPOSED` sibling), **or** was explicitly, auditably closed at zero requirements by at least one resolved (non-`UNRESOLVED`) `SemanticAdjudication` — distinct from a candidate never examined at all.
- **`PARTIAL`** — some governing requirements exist, no `REVIEW_PROPOSED` fact remains pending, but technical-truth coverage is incomplete.
- **`ADJUDICATION_REQUIRED`** — at least one `REVIEW_PROPOSED` `CandidateFactRequirement` targeting this candidate has not (yet) been promoted by an active governing adjudication.
- **`UNRESOLVED`** — a mastery-bearing leaf candidate with zero governing `requiredFactKeys`, no governed child decomposition, and **no adjudication history at all** — the exact false-green case finding F names. Emits `KNOWLEDGE_DECOMPOSITION_GAP` (§22.3).
