...
### 21.6 CC-20A — `supportingEvidenceRefs` resolve mechanically against real accepted evidence

CC-20 introduced `supportingEvidenceRefs` but did not mechanically validate it — a fabricated `evidenceId`, a real `evidenceId` cited under the wrong role, or a real ref that simply doesn't pertain to the target candidate all validated merely because `sourceRef`/`sourceLocator` were non-empty and a matching `CandidateFactRequirement` proposal existed. CC-20A closes this: `validateSemanticAdjudications` now resolves every `supportingEvidenceRefs` entry, by `(role, evidenceId)`, against the **same** validated evidence streams/registries the rest of `buildStandardPipeline` already produces — never a second, ungoverned evidence registry:
...
**Governing support gate.** For `REQUIRED_CORE`/`REQUIRED_OPERATIONAL` only: at least one resolved ref must be a `CURRICULUM_MATCH` or `ASSESSMENT_MATCH` for the exact target candidate, **and** every basis in `adjudicationBasis` must itself be compatible with what actually resolved (`isBasisCompatibleWithSupport`) — `PUBLIC_ASSESSMENT_CALIBRATION` requires an `ASSESSMENT_MATCH` specifically; `COMMAND_VERB_AND_LEVEL` requires both a `CURRICULUM_MATCH` **and** a `QUALIFICATION_LEVEL_MATCH`; `SEMANTIC_NECESSITY`/`OPERATIONAL_NECESSITY` require a `CURRICULUM_MATCH` or `ASSESSMENT_MATCH` (a `TECHNICAL_TRUTH_REAL`-only ref may corroborate but never substitutes); `MULTIPLE_VALID_IMPLEMENTATIONS`/`REPRESENTATIVE_EXEMPLAR_SELECTION`/`OVERDEPTH_FORMALISM` require at least one genuinely resolved (non-fabricated) ref of any kind, since they are non-mastery bases by construction; `INSUFFICIENT_EVIDENCE` can never be compatible with a governing decision. Failing either half of this gate rejects the whole adjudication — reported as `EVIDENCE_NORMALIZATION_REVIEW` (never silently dropped) — regardless of `adjudicatorKind`: the label never bypasses the gate. Non-governing decisions (`CONTEXT_ONLY`/`REJECT_*`/`REPRESENTATIVE_EXEMPLAR`/`UNRESOLVED`) are not subject to this specific gate, since they never promote anything in the first place.
