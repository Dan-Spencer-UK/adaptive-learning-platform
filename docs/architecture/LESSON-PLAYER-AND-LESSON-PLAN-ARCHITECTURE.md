...
## 21. Governed rich teaching content blocks within a step (Remediation Package 2, CC-13C.2B/2D)

**Root cause this section closes:** `reports/learning-package-pipeline-audit/LESSON-DEPTH-AND-FRAGMENTATION-REGISTER.md` §3 found that no field in `lessonStepSchema` could hold authored multi-sentence/multi-paragraph teaching prose — a step's body copy was reconstructed only from deduplicated one-sentence assertion `statement` strings. `packages/content-schema/src/lesson-plan.ts` (commit `0e220ee`, "feat: add governed rich teaching content blocks") closes that gap with an optional, structured `contentBlocks` representation. **This section records the implemented architecture; it does not itself grant any real lesson adoption — see the adoption note at the end of this section.**

**A `LessonStep` remains what §4 already establishes: a semantic learning section, not a viewport.** `contentBlocks` does not change that — it gives one section a governed way to hold more than one short paragraph of teaching prose, composed with existing visuals/formulas/worked examples, and it scrolls through the existing `ScrollableLessonStep.tsx` exactly as any other step's content already does (§4). This package did not modify `ScrollableLessonStep.tsx` — it remains the one scrolling container for step content, unaware of whether a step renders via `contentBlocks` or the legacy path.

**Presence/absence is the migration switch, and it is load-bearing:**
- `contentBlocks` is optional on `LessonStep`. **Absent** → the step renders through the pre-existing legacy path (`resolveBodyStatements()` + `representation.*` in `resolve-lesson-step.ts`), completely unchanged by this package.
- **Present** → `contentBlocks` is the SOLE authoritative learner-visible representation for that step, rendered in exactly authored order (never resorted/regrouped by type). The legacy `bodyStatements` resolution is not also computed for that step.
- An explicit empty array (`contentBlocks: []`) is schema-invalid (`.min(1)`) — there is no such thing as "present but empty."

**`learnerFacingHeading`** is a new, optional, plain-text learner-facing section heading (`z.string().min(1).optional()`), rendered by `LessonStepView.tsx` with native accessibility heading semantics, no nested heading levels, no heading block system. It is legacy-neutral: a step without one renders exactly as before, using its existing pedagogical-role-derived `sectionLabel`.

**`semanticUnit` is unaffected and remains governance/authoring metadata only** (CC-13A) — a coherent-teaching-unit label used by fragmentation-review tooling, never rendered to the learner. `learnerFacingHeading` and `semanticUnit` are deliberately distinct fields serving different audiences (learner vs. reviewer); this package did not merge or alias them.

**Exactly six V1 block families**, a discriminated union on `type`, no others: `paragraph` (plain text, no Markdown/HTML/inline spans), `list` (ordered/unordered, flat, no nesting), `visual` (see below), `formula` (references an existing `FormulaFamily` by id), `worked_example` (references an existing `WorkedExampleBlueprint` by id), `callout` (`key_point` | `definition` | `caution`, plain text). No `subheading` block, no rich-text span system, no arbitrary/unrestricted markup of any kind — every governed-content-bearing block reuses an existing governed reference, never a new parallel content type.

**Visual blocks in this package support only the two visual forms the runtime already renders today** — a deterministic `diagram` (`DiagramBlueprint` reference) or a governed `visual_aid` (`VisualAidBlueprint` reference). Produced-artwork integration (ADR-0005's `VisualRequirement` → `ReferenceDossier` → `ProductionVisualAsset` authority chain) is **deliberately not part of this package** — it is Packages 3-5's later work (storyboard/visual-opportunity planning is Package 3, not yet started — see `reports/learning-package-pipeline-audit/REMEDIATION-PLAN.md`). This section must not be read as implying produced artwork can currently be referenced from a `contentBlocks` visual block; it cannot.

**A rich teaching step and a graded/evidence-bearing question step must not be the same step.** `lessonPlanSchema`'s `superRefine` rejects a step that declares `contentBlocks` alongside either `completionCondition: "correct_answer_required"` or a `questionBlueprintId` — rich teaching and graded questioning stay on separate semantic steps, exactly as the existing DO → RESPOND → FEEDBACK → NEXT model (§8) already implies. The same gate also rejects any legacy `representation.*` field coexisting with `contentBlocks` (mutual exclusivity), and requires a `contentBlocks` step to declare `mayRevealTargetAnswer` explicitly (`true` or `false`, never left undefined) — teaching content may reveal answer-bearing material, but only by explicit declaration, never by the implicit "absent means false" convention legacy steps rely on.

**Runtime flow (production-wiring proved, see below):** `apps/mobile/src/lib/lesson-content/resolve-lesson-step.ts`'s `resolveLessonStep()` resolves `contentBlocks` (when present) into `ResolvedContentBlock[]`, with every governed reference resolved to the real object (never left as a bare id). `apps/mobile/src/components/lesson/LessonStepView.tsx` renders `learnerFacingHeading` and maps each resolved block to `apps/mobile/src/components/lesson/ContentBlockView.tsx`, which dispatches per block type, reusing the same governed representation components (`FormulaEquation`, `WorkedSubstitution`, `VirTriangle`, `DiagramRenderer`) the legacy path already uses. Knowledge grounding is unchanged: it remains at step/section level via the existing `teaches`/`reinforces`/`tests`/`capabilityIds` fields — `contentBlocks` introduces no new per-paragraph/per-block assertion-grounding mechanism, and mechanical validation (schema + `computeLessonContentDependencies`) proves governed-reference integrity, never the semantic correctness of authored prose.

**Production-wiring proof (CC-13C.2D, commit `1087634`):** `apps/mobile/src/lib/lesson-content/rich-content-production-wiring.test.tsx` proves the above chain end-to-end through the REAL mobile lesson-content lookup boundary — `lesson-player.tsx` → `getLocalLesson()`/`getLocalLessonFrom(MOBILE_CONTENT_PROJECTION, ...)` → `state.record.lesson` → `resolveLessonStep()` → `LessonStepView` → `ContentBlockView` — using a synthetic release-member lesson, never a lesson object hand-constructed and passed directly to the resolver (the risk every earlier Package-2 unit test individually left open). No mapping/serialization/reconstruction layer in that chain strips `learnerFacingHeading` or `contentBlocks`; `@alp/learning-engine`'s `LessonInstance` (the per-learner session-sequencing object) is reference-only by design (step ids only) and never reconstructs step content, so it cannot be a stripping point either.

**Adoption status — read this before assuming any lesson uses this representation:** as of this section, **zero real lessons in the bundled Unit 202 corpus (or any content release) declare `contentBlocks` or `learnerFacingHeading`.** This package is schema/runtime-capable, not content-adopted. Re-authoring real lesson content to use this representation is later, separately-gated work (`REMEDIATION-PLAN.md` Package 13, strictly gated on the full V1 pilot passing) — it is not authorised or implied by this section.
