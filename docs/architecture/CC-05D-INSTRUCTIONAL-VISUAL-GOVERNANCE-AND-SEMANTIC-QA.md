...
## U. Relationship to reference governance and V1 scope (CC-13A, ADR-0005/ADR-0006)

**Semantic QA follows reference selection; it never substitutes for it.** This document's mechanical (§E) and semantic (§F/§G) QA pipelines validate a visual *after* it has been produced from an already-approved reference. They do not, and cannot, establish that the reference used was the right one to select — that is an independent, upstream decision governed by [`docs/governance/VISUAL-REFERENCE-REVIEW-PROTOCOL.md`](../governance/VISUAL-REFERENCE-REVIEW-PROTOCOL.md) (Claude → ChatGPT candidate handoff, ChatGPT independent reference review/approval, ChatGPT → Claude production handoff) and, for the exact reference frame, the semantic reference QA gate in [`PREMIUM-INSTRUCTIONAL-VISUAL-PRODUCTION-PIPELINE.md`](PREMIUM-INSTRUCTIONAL-VISUAL-PRODUCTION-PIPELINE.md) §22. A visual passing every check in this document while having been produced from a wrongly-selected or never-independently-reviewed reference is not learner-ready — CC-05D governs "was this rendered correctly against its contract," never "was the right reference chosen."

**V1 visual completeness (ADR-0006).** For a V1 ordinary lesson, "visual completeness" (§Q, and `PREMIUM-INSTRUCTIONAL-VISUAL-PRODUCTION-PIPELINE.md` §12) is judged against the canonical lesson's own visual-opportunity analysis (its one canonical route's teaching needs) and the dedicated formative/mock assessment's visual needs (§9 of the style guide; teaching/assessment/feedback states) — not against every visual variant a richer, post-V1 adaptive branch might hypothetically need. Visual needs specific to retained-but-deferred adaptive/remediation branches (`LESSON-PLAYER-AND-LESSON-PLAN-ARCHITECTURE.md` §7/§20) are not a V1 completeness requirement.
