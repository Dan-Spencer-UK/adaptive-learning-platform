...
**Implementation (CC-11.5):** [`tools/visual-production-studio/`](../../tools/visual-production-studio/README.md) (`npm run visuals:studio`) is the manual-assisted production implementation of this workflow -- a local development tool, not learner-facing product functionality. It holds the structured production catalogue (governed concept, curated reference, immutable facts, creative freedoms, prohibited changes per visual family), deterministically builds each visual's copyable production prompt from that data, and provides the paste/drop → preview → validate → safe-save → provenance-record mechanics for TECHNICAL VERIFICATION through REAL PIXEL REVIEW once artwork is produced in a separate ChatGPT session. It does not call any image-generation API itself and does not perform ALP REDESIGN / ILLUSTRATION -- a human (Product Owner) remains the actual illustrator/reviewer at every step, exactly as this section requires.
...
- The production tooling for that future work now exists (CC-11.5: [`tools/visual-production-studio/`](../../tools/visual-production-studio/README.md), `npm run visuals:studio`), seeded with a 24-entry Unit 202 production catalogue -- but **zero premium/hybrid artwork has actually been produced or approved through it yet**. The tool being built is not evidence that the artwork exists.
