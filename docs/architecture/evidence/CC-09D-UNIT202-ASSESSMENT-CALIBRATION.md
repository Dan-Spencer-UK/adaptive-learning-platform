...
>
> **Amendment (CC-09F, 2026-08-22):** an independent Project Architect Phase-1 audit found that §A's "no later revision found" currency claim, while accurate at the time, had not been re-checked since. City & Guilds has since republished the question-paper document (not the mark scheme) as **v1.2, dated 14 April 2026** -- confirmed directly from the live qualification page. The new edition was retrieved and registered per ADR-0002 (`SV_CG_602_SAMPLE_QUESTIONS_V1_2`, fingerprint `71a39b800f4048d2775529ec4783218bf808e7478561214d40f7a61538f39669`), and the v1.0 record this report describes was reclassified `SUPERSEDED` (a bibliographic fact, not a validity judgement -- this report's own findings remain a valid historical snapshot). **The retrieved v1.2 PDF is encrypted and could not be opened for text extraction by any tool available this session** (Poppler `pdftotext`/`pdfinfo` and `pdfjs-dist` all fail with a password/decryption error against the same reproducibly-fingerprinted artefact) -- so the item-level comparison against this report's 40-item analysis (§B/§C below) that would normally follow a new edition could **not** be performed. No finding, assertion, or `assessmentStyleEvidence` citation in this report or in CC-09E/E.1/E.2 was changed as a result: doing so without being able to read v1.2 would itself be an unevidenced assumption. This is recorded as an open item requiring either an alternative retrieval/extraction method or independent Project Architect verification with full PDF access -- see `PROJECT-STATUS.md` §CC-09F for the full account.
